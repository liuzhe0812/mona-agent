...
pub async fn hoard_search(
    ...
    let q_pattern = format!("%{}%", q_lower);
    let limit = limit.unwrap_or(20);

    let sql = if source.is_some() {
        format!(
            "SELECT {} FROM hoards WHERE source = ? AND (LOWER(title) LIKE ? OR LOWER(summary) LIKE ? OR LOWER(tags) LIKE ? OR LOWER(COALESCE(content,'')) LIKE ?) ORDER BY (CASE WHEN LOWER(title) LIKE ? THEN 3 ELSE 0 END + CASE WHEN LOWER(tags) LIKE ? THEN 2 ELSE 0 END + CASE WHEN LOWER(summary) LIKE ? THEN 2 ELSE 0 END + CASE WHEN LOWER(COALESCE(content,'')) LIKE ? THEN 1 ELSE 0 END) * source_strength DESC LIMIT ?",
            SELECT_COLS
        )
    } else {
        format!(
            "SELECT {} FROM hoards WHERE LOWER(title) LIKE ? OR LOWER(summary) LIKE ? OR LOWER(tags) LIKE ? OR LOWER(COALESCE(content,'')) LIKE ? ORDER BY (CASE WHEN LOWER(title) LIKE ? THEN 3 ELSE 0 END + CASE WHEN LOWER(tags) LIKE ? THEN 2 ELSE 0 END + CASE WHEN LOWER(summary) LIKE ? THEN 2 ELSE 0 END + CASE WHEN LOWER(COALESCE(content,'')) LIKE ? THEN 1 ELSE 0 END) * source_strength DESC LIMIT ?",
            SELECT_COLS
        )
    };
    ...
    let rows = if let Some(src) = source {
        stmt.query_map(
            params![src, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, limit],
            row_to_item,
        )
        .map_err(|e| format!("Failed to query: {}", e))?
    } else {
        stmt.query_map(
            params![q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, q_pattern, limit],
            row_to_item,
        )
        .map_err(|e| format!("Failed to query: {}", e))?
    };
