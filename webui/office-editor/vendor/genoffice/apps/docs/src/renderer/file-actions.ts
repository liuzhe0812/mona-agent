 ...
 */
export async function buildDocBytes(ctx: FileActionContext, content?: PmNode): Promise<Uint8Array | null> {
  const { doc, editor } = ctx
  if (!doc || !editor) return null
  const plan = pmDocToSavePlan(content ?? editor.getJSON() as PmNode, doc.parsed.blocks)
