...
export interface EmbeddedDocsApi {
  editor: Editor
  open: (result: Exclude<OpenDocxResult, null>) => Promise<unknown>
  buildBytes: () => Promise<Uint8Array | null>
  fileName: () => string | null
  snapshot: () => () => Promise<Uint8Array | null>
  pageSettings: () => SectionSettings[]
  setPageSettings: (index: number, settings: SectionSettings) => void
  readHeaderFooter: () => EmbeddedHeaderFooterState
  setHeaderFooter: (kind: 'header' | 'footer', value: HeaderFooter, view?: HfView) => void
  restoreHeaderFooter: (state: EmbeddedHeaderFooterState) => void
}

export interface EmbeddedHeaderFooterState {
...
interface AiHeaderFooterRuntime {
  overlay: Map<string, HeaderFooter>
  overlayKey: (kind: 'header' | 'footer', view: HfView) => string
  valueOf: (kind: 'header' | 'footer', view: HfView) => HeaderFooter | null
  commit: (kind: 'header' | 'footer', next: HeaderFooter, view?: HfView) => void
  titlePg: boolean
  evenOddHf: boolean
  multiHf: boolean
  locked: boolean
...
function applyHeaderFooterOverlay(
  state: EmbeddedHeaderFooterState,
  overlay: ReadonlyMap<string, HeaderFooter>,
): void {
  if (overlay.size === 0) return
  state.hfVariants = { ...state.hfVariants }
  state.hfVariantsDirty = [...state.hfVariantsDirty]
  state.sectionHfEdits = { ...state.sectionHfEdits }
  for (const [key, value] of overlay) {
    if (key === 'header:default') {
      state.header = value
      state.headerDirty = true
    } else if (key === 'footer:default') {
      state.footer = value
      state.footerDirty = true
    } else if (key === 'header:first' || key === 'footer:first' || key === 'header:even' || key === 'footer:even') {
      const variant = key === 'header:first'
        ? 'headerFirst'
        : key === 'footer:first'
          ? 'footerFirst'
          : key === 'header:even'
            ? 'headerEven'
            : 'footerEven'
      state.hfVariants[variant] = value
      if (!state.hfVariantsDirty.includes(variant)) state.hfVariantsDirty.push(variant)
    } else if (key.startsWith('section:')) {
      const [, lastBlockIndex, kind] = key.split(':')
      if (lastBlockIndex && (kind === 'header' || kind === 'footer')) {
        state.sectionHfEdits[`${lastBlockIndex}:${kind}`] = value
      }
    }
  }
}

export interface EmbeddedDocsController {
  extensions?: Array<(typeof editorExtensions)[number]>
  attach: (api: EmbeddedDocsApi) => void | (() => void)
  changed?: (target: string) => void
}

export function App({ embeddedController }: { embeddedController?: EmbeddedDocsController } = {}) {
  ...
  const [showAi, setShowAi] = useState(() => !embeddedController && localStorage.getItem('aidocs.showAi') !== '0')
  ...
  const commitHf = (kind: 'header' | 'footer', next: HeaderFooter, viewOverride?: HfView) => {
    ...
    if (view === 'default' && multiHf && hfSectionClamped < sections.length - 1) {
      ...
      embeddedController?.changed?.(`document:${kind}`)
      return
    }
    if (view === 'default') {
      if (kind === 'header') {
  ...
  const [autoSave, setAutoSave] = useState(
    () => !embeddedController && localStorage.getItem('aidocs.autoSave') === '1',
  )
  ...
  const editor = useEditor({
    extensions: [...editorExtensions, ...(embeddedController?.extensions ?? [])],
  ...
  useEffect(
    () =>
      subscribeSubEditorState((docChanged) => {
        if (docChanged) {
          dirtyRef.current = true
          embeddedController?.changed?.('document:textbox')
        }
        forceRender()
      }),
    [embeddedController],
  ...
  }, [embeddedController, showAi])

  useEffect(() => {
    if (embeddedController) return
    localStorage.setItem('aidocs.autoSave', autoSave ? '1' : '0')
  }, [autoSave, embeddedController])
  ...
  useEffect(() => {
    document.title = doc ? doc.fileName : embeddedController ? 'Mona 文档编辑器' : 'GenOffice Docs'
  }, [doc, embeddedController])
  ...
  useEffect(() => {
    if (tornDown || embeddedController) return
    const timer = window.setInterval(() => {
      void writeRecoveryCopyImpl(fileCtxRef.current)
    }, 30_000)
    return () => window.clearInterval(timer)
  }, [embeddedController, tornDown])
  ...
  const aiHfCtxRef = useRef<AiHeaderFooterRuntime | null>(null)
  fileCtxRef.current = {
    editor,
    doc,
    dirtyRef,
  ...
  useEffect(() => {
    if (!embeddedController || !editor) return
    return embeddedController.attach({
      editor,
      open: (result) => loadFile(result),
      buildBytes: () => buildDocBytes(fileCtxRef.current),
      fileName: () => fileCtxRef.current.doc?.fileName ?? null,
      snapshot: () => {
        const context = { ...fileCtxRef.current }
        const aiHf = aiHfCtxRef.current
        if (aiHf) {
          const state: EmbeddedHeaderFooterState = {
            header: context.header,
            headerDirty: context.headerDirty,
            footer: context.footer,
            footerDirty: context.footerDirty,
            hfVariants: context.hfVariants,
            hfVariantsDirty: context.hfVariantsDirty,
            sectionHfEdits: context.sectionHfEdits,
            titlePg: aiHf.titlePg,
            titlePgDirty: context.titlePgDirty || aiHf.titlePg !== context.titlePg,
            evenOddHf: aiHf.evenOddHf,
            evenOddHfDirty: context.evenOddHfDirty || aiHf.evenOddHf !== context.evenOddHf,
          }
          applyHeaderFooterOverlay(state, aiHf.overlay)
          Object.assign(context, state)
        }
        const content = editor.getJSON()
        return () => buildDocBytes(context, content as Parameters<typeof buildDocBytes>[1])
      },
      pageSettings: () => {
        const context = fileCtxRef.current
        return structuredClone(context.sections.length
          ? context.sections.map((item) => item.settings)
          : [context.section ?? DEFAULT_SECTION])
      },
      setPageSettings: (index, settings) => {
        const context = fileCtxRef.current
        if (!Number.isInteger(index) || index < 0 || index >= Math.max(1, context.sections.length)) {
          throw new Error('找不到指定的文档节。')
        }
        context.sections = context.sections.map((item, at) => at === index ? { ...item, settings } : item)
        context.setSections(context.sections)
        if (index === Math.max(0, context.sections.length - 1)) {
          context.section = settings
          context.sectionDirty = true
          context.setSection(settings)
          context.setSectionDirty(true)
        } else {
          context.sectionsDirty = [...new Set([...context.sectionsDirty, index])]
          context.setSectionsDirty(context.sectionsDirty)
        }
        embeddedController.changed?.('document:section')
      },
      readHeaderFooter: () => {
        const context = fileCtxRef.current
        const state: EmbeddedHeaderFooterState = {
          ...
          hfVariantsDirty: context.hfVariantsDirty,
          sectionHfEdits: context.sectionHfEdits,
          titlePg: context.titlePg,
          titlePgDirty: context.titlePgDirty,
          evenOddHf: context.evenOddHf,
          evenOddHfDirty: context.evenOddHfDirty,
        }
        const aiHf = aiHfCtxRef.current
        if (aiHf) {
          state.titlePg = aiHf.titlePg
          state.titlePgDirty = state.titlePgDirty || aiHf.titlePg !== context.titlePg
          state.evenOddHf = aiHf.evenOddHf
          state.evenOddHfDirty = state.evenOddHfDirty || aiHf.evenOddHf !== context.evenOddHf
          applyHeaderFooterOverlay(state, aiHf.overlay)
        }
        return structuredClone(state)
      },
      setHeaderFooter: (kind, value, view = 'default') => {
        const aiHf = aiHfCtxRef.current
        if (!aiHf) throw new Error('页眉页脚通道尚未准备好。')
        const context = fileCtxRef.current
        if (view === 'first' && !aiHf.titlePg) {
          context.setTitlePg(true)
          context.setTitlePgDirty(true)
          aiHf.titlePg = true
        }
        if (view === 'even' && !aiHf.evenOddHf) {
          context.setEvenOddHf(true)
          context.setEvenOddHfDirty(true)
          aiHf.evenOddHf = true
        }
        aiHf.commit(kind, value, view)
        aiHf.overlay.set(aiHf.overlayKey(kind, view), value)
      },
      restoreHeaderFooter: (state) => {
        aiHfCtxRef.current?.overlay.clear()
        const context = fileCtxRef.current
        context.header = state.header
        context.headerDirty = state.headerDirty
        context.footer = state.footer
        context.footerDirty = state.footerDirty
        context.hfVariants = state.hfVariants
        context.hfVariantsDirty = state.hfVariantsDirty
        context.sectionHfEdits = state.sectionHfEdits
        context.titlePg = state.titlePg
        context.titlePgDirty = state.titlePgDirty
        context.evenOddHf = state.evenOddHf
        context.evenOddHfDirty = state.evenOddHfDirty
        context.setHeader(state.header)
        context.setHeaderDirty(state.headerDirty)
        context.setFooter(state.footer)
        context.setFooterDirty(state.footerDirty)
        context.setHfVariants(state.hfVariants)
        context.setHfVariantsDirty(state.hfVariantsDirty)
        context.setSectionHfEdits(state.sectionHfEdits)
        context.setTitlePg(state.titlePg)
        context.setTitlePgDirty(state.titlePgDirty)
        context.setEvenOddHf(state.evenOddHf)
        context.setEvenOddHfDirty(state.evenOddHfDirty)
      },
    })
  }, [editor, embeddedController, loadFile])
  ...
  const applyProtectDialog = async (result: ProtectDialogResult) => {
    ...
    if (changed) {
      setStatus(t('appProtectUpdated'))
      embeddedController?.changed?.('document:protection')
    }
  }
  ...
  const applyPgNumFormat = useCallback(() => {
    ...
    embeddedController?.changed?.('document:page-number-format')
  }, [pgNumModal, sections, activeSection, embeddedController])
  ...
  const submitNote = useCallback((text: string) => {
    const existingNote = reviewCtxRef.current.notePrompt?.id !== undefined
    submitNoteImpl(reviewCtxRef.current, text)
    if (existingNote) embeddedController?.changed?.('document:notes')
  }, [embeddedController])
  ...
  const resolveComment = useCallback(
    (id: string, done: boolean) => {
      ...
      embeddedController?.changed?.('document:comments')
    },
    [embeddedController],
  )
  ...
  const addInk = useCallback(
    (annotation: InkAnnotation) => {
      addInkImpl(reviewCtxRef.current, annotation)
      embeddedController?.changed?.('document:ink')
    },
    [embeddedController],
  )
  const removeInks = useCallback((ids: string[]) => {
    removeInksImpl(reviewCtxRef.current, ids)
    if (ids.length > 0) embeddedController?.changed?.('document:ink')
  }, [embeddedController])
  const clearInks = useCallback(() => {
    if (inkAnnotations.length > 0) {
      clearInksImpl(reviewCtxRef.current)
      embeddedController?.changed?.('document:ink')
      return
    }
    clearInksImpl(reviewCtxRef.current)
  }, [embeddedController, inkAnnotations.length])
  ...
  }, [autoSave, doc, embeddedController, save])

  // autosave: every 30s and on window blur, silently persist pending changes
  useEffect(() => {
    if (embeddedController || tornDown || !autoSave || !doc || !doc.filePath) return
    const tick = () => {
  ...
  }, [autoSave, doc, editor, embeddedController, save, tornDown])
  ...
  }, [editor, embeddedController, save])
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 's') {
        e.preventDefault()
        if (!embeddedController) void save(e.shiftKey)
      }
      if ((e.metaKey || e.ctrlKey) && e.key === 'o') {
        e.preventDefault()
        if (!embeddedController) void openFile()
      }
      ...
      if (e.key === 'F7' && !e.shiftKey && doc && !embeddedController) {
        e.preventDefault()
        runAiProofread()
      }
    }
    ...
    return window.desktop.onMenuCommand((command, payload) => {
      ...
      switch (command) {
        ...
        case 'open':
          ...
          break
        case 'open-path':
          if (!embeddedController && payload) void openRecent(payload)
          break
        case 'save':
          if (!embeddedController) void save(false)
          break
        case 'save-as':
          if (!embeddedController) void save(true)
          break
        case 'undo':
  ...
  const aiHfCtx = {
    ...
    overlayKey(kind: 'header' | 'footer', view: HfView): string {
      if (view === 'default' && multiHf && hfSectionClamped < sections.length - 1) {
        const sec = sections[hfSectionClamped]
        if (sec) return `section:${sec.lastBlockIndex}:${kind}`
      }
      return `${kind}:${view}`
    },
    valueOf(kind: 'header' | 'footer', view: HfView): HeaderFooter | null {
      const pending = this.overlay.get(this.overlayKey(kind, view))
  ...
  const aiHfAccess = useMemo<AiHeaderFooterAccess>(
    () => ({
      ...
      set: (kind, view, text) => {
        ...
        ctx.overlay.set(ctx.overlayKey(kind, view), next)
        return null
      },
    }),
    [],
  ...
  const ribbonActions = useStableCallbacks({
    ...
    onSection: (next: SectionSettings) => {
      const context = fileCtxRef.current
      context.sections = context.sections.map((item, index) => index === activeSection ? { ...item, settings: next } : item)
      if (sections.length <= 1 || activeSection === sections.length - 1) {
        context.section = next
        context.sectionDirty = true
      } else {
        context.sectionsDirty = [...new Set([...context.sectionsDirty, activeSection])]
      }
    ...
    onAddSource: (source: SourceInfo) => {
      ...
      embeddedController?.changed?.('document:sources')
    },
    headingPages,
    onZoom: setZoom,
    onZoomFit: zoomFit,
    ...
    onHeader: (next: HeaderFooter) => {
      setHeader(next)
      setHeaderDirty(true)
      embeddedController?.changed?.('document:header')
    },
    ...
    onFooter: (next: HeaderFooter) => {
      setFooter(next)
      setFooterDirty(true)
      embeddedController?.changed?.('document:footer')
    },
    onTitlePg: (on: boolean) => {
      toggleTitlePg(on)
      embeddedController?.changed?.('document:first-page')
    },
    onEvenOddHf: (on: boolean) => {
      ...
      embeddedController?.changed?.('document:odd-even-headers')
    },
  ...
  const quickActions = useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [autoSave, embeddedController, hasDoc, hasUnsavedChanges, editor, save, lang, histState],
  )
  ...
  return (
    <div
      className={`app ${embeddedController ? 'mona-embedded' : ''} ${readMode ? 'read-mode' : ''}${revisionDisplay !== 'all' ? ` rev-display-${revisionDisplay}` : ''}${revisionDisplay === 'all' && viewMode === 'print' ? ' rev-balloon' : ''}`}
    >
      <ToastHost />
