...
export interface EmbeddedSlidesController {
...
export function App({ embeddedController }: { embeddedController?: EmbeddedSlidesController } = {}) {
  const { lang } = useI18n()
  ...
  useEffect(() => {
    if (!embeddedController && slides.length) void syncPrivateFonts()
  }, [embeddedController, slides])
  ...
  const refreshMissingFonts = useCallback(() => {
    if (embeddedController || missingFontsDismissed.current) return
    window.slidesApi
      .fontMissing?.()
  ...
  useEffect(
    () => {
      if (embeddedController) return undefined
      return window.slidesApi.onFontsChanged?.(() => {
        void syncPrivateFonts()
        refreshMissingFonts()
      })
    },
    [embeddedController, refreshMissingFonts],
  )
  ...
  useEffect(() => {
    embeddedController?.selectionChanged?.({ slideIndex: current, sourceIds: [...selectedIds] })
  }, [embeddedController, current, selectedIds])
  ...
  const [autoSave, setAutoSave] = useState(
    () => !embeddedController && localStorage.getItem('ai-slides-auto-save') === '1',
  )
  useEffect(() => {
  ...
  const [showAi, setShowAi] = useState(
    () => !embeddedController && localStorage.getItem('ai-slides-show-ai') !== '0',
  )
  const [embeddedAiMenuOpen, setEmbeddedAiMenuOpen] = useState(false)
  const embeddedAiMenuRef = useRef<HTMLDivElement | null>(null)
  useEffect(() => {
    if (!embeddedAiMenuOpen) return
    const dismiss = (event: PointerEvent) => {
      if (!embeddedAiMenuRef.current?.contains(event.target as Node)) setEmbeddedAiMenuOpen(false)
    }
  ...
  useEffect(() => {
    if (embeddedController || !autoSave || !path) return
    let saving = false
    const tick = () => {
  ...
  }, [autoSave, embeddedController, path, editing, editingCell, save])
  ...
  // Global shortcuts (keyboard-actions.ts): the handler reads the latest state via ctxRef.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!embeddedController) void window.slidesApi.getAiSettings().then(setAiSettings)
  }, [embeddedController])
  ...
  const toggleAi = useCallback(() => {
    if (embeddedController) {
      embeddedController.requestAi({
        prompt: `请分析当前打开的演示文稿第 ${current + 1} 页，并根据现有内容提出可以直接执行的优化。`,
        displayText: '使用 Mona AI 优化当前演示文稿',
      })
      return
    }
    setShowAi((v) => {
  ...
  const pushAiPreset = useCallback(
    (
      ...
      if (embeddedController) {
        embeddedController.requestAi({
          prompt: `当前目标是演示文稿第 ${current + 1} 页。${text}`,
  ...
  return (
    <div className={`app${embeddedController ? ' mona-embedded' : ''}`}>
      <ToastHost />
      <Ribbon
        embedded={Boolean(embeddedController)}
        hasDoc={!!slide}
        deckEmpty={deckEmpty}
      ...
      <div className="app-main">
        {!embeddedController && slide && viewMode !== 'reading' && viewMode !== 'sorter' && (
        ...
        <div className="app-content">
          {!embeddedController && missingFonts.length > 0 && (
                    ...
                    is sized to the scaled dimensions to give the scroller the real extent */}
                      ...
                      >
                        <div className="stage-ai-bar">
                          {embeddedController ? (
                            <div className="mona-stage-ai-menu" ref={embeddedAiMenuRef}>
                              <button
                                type="button"
                                className="stage-ai-btn mona-stage-ai-trigger"
                                aria-haspopup="menu"
                                aria-expanded={embeddedAiMenuOpen}
                                onClick={() => setEmbeddedAiMenuOpen((open) => !open)}
                              >
                                <GensparkMark size={14} />
                                <span>AI 编辑</span>
                              </button>
                              {embeddedAiMenuOpen ? (
                                <div className="mona-stage-ai-actions" role="menu">
                                  <button
                                    type="button"
                                    role="menuitem"
                                    onClick={() => {
                                      setEmbeddedAiMenuOpen(false)
                                      toggleAi()
                                    }}
                                  >
                                    优化当前页
                                  ...
                                  {!deckEmpty ? (
                                    <>
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          setEmbeddedAiMenuOpen(false)
                                          pushAiPreset(t('aiFactCheckPrompt'))
                                        }}
                                      >
                                        {t('aiFactCheckBtn')}
                                      </button>
