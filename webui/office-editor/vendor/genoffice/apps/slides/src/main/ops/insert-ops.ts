  ...
  apply(op, ctx): OpRecord {
    ...
    if (!el) {
      throw new GuidedError(
        `op "addPicture": unsupported image format ".${String(op.ext)}" (png/jpg/jpeg/gif/bmp/webp/svg/tif supported).`,
      )
    }
    return { op, created: [el.id] }
  },
...
register({
  ...
  apply(op, ctx): OpRecord {
    ...
    const r = addChart(ctx.opened, index, {
      ...
      offset: reqRect(op),
      ...(['legendPos', 'dataLabels', 'gridlines', 'catAxisTitle', 'valAxisTitle', 'gapWidthPct', 'pointColors'] as const)
        .reduce<Partial<NewChartOptions>>((options, key) => (
          op[key] === undefined ? options : { ...options, [key]: op[key] }
        ), {}),
    })
