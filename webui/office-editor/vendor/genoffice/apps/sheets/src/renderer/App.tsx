...
export interface EmbeddedSheetsApi {
  runtime: UniverRuntime
  open: (file: WorkbookFile) => void
}

export interface EmbeddedSheetsController {
  attach: (api: EmbeddedSheetsApi) => void | (() => void)
  save?: () => void | Promise<void>
}

export function App({ embeddedController }: { embeddedController?: EmbeddedSheetsController } = {}): React.JSX.Element {
  ...
  const [autoSave, setAutoSave] = useState(
    () => !embeddedController && localStorage.getItem('ai-sheets-auto-save') === '1',
  )
  // Ref mirror for callbacks captured when an AI run starts
  const autoSaveRef = useRef(autoSave)
  autoSaveRef.current = !embeddedController && autoSave
  useEffect(() => {
  ...
  useEffect(() => {
    if (embeddedController || !autoSave) return
    let saving = false
    const tick = () => {
  ...
  const [message, setMessage] = useState(embeddedController ? '就绪' : t('appReadyInitial'))
  ...
  if (!embeddedController && !agentLoopRef.current) {
  ...
  useEffect(() => {
    if (!embeddedController) void window.desktopApi.getAiSettings().then(setAiSettingsState)
  }, [embeddedController])
  ...
  useEffect(() => {
    ...
    const detachEmbedded = embeddedController?.attach({ runtime, open: openLazyWorkbook })
    ...
    if (!embeddedController) void window.desktopApi?.consumeNewBlankWorkbook?.()
  ...
  async function autoSaveCompletedAiRun(): Promise<void> {
    ...
    aiApplyPromisesRef.current = []
    if (applies.length === 0 || embeddedController) return
  ...
  return (
    <>
      ...
      <ExcelShell
        ...
        onSave={() => void (embeddedController?.save?.() ?? handleSave('save'))}
        canSaveAs={workbookFile !== null}
        onSaveAs={() => void (embeddedController?.save?.() ?? handleSave('save-as'))}
        onRedo={handleRedo}
        autoSave={autoSave}
