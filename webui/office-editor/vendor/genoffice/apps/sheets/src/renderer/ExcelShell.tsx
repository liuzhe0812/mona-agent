...
export function ExcelShell({
  ...
  const [isCopilotOpen, setIsCopilotOpen] = useState(
    () => !embedded && localStorage.getItem('ai-sheets-show-ai') !== '0',
  )
  useEffect(() => {
  ...
  return (
    <main className={`app-shell ${isCopilotOpen ? '' : 'copilot-collapsed'}`}>
      <header className="excel-header">
        ...
        <Ribbon
          ...
          onCommand={(command) => {
            ...
            else if (command === 'ai-open-panel' && !embedded) setIsCopilotOpen(true)
            else if (command === 'ai-toggle-panel' && !embedded) setIsCopilotOpen((v) => !v)
      ...
      <div className="sheet-body">
        ...
        />}
        <div className={`sheet-main${embedded ? ' mona-embedded-bottom-bar' : ''}`}>
          ...
          {!embedded && aiSelectionAskAnchor && aiScopeRange && !aiBusy && (
            <AiSelectionAsk
