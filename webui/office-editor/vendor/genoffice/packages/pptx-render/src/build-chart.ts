...
function axisLabelColor(
  model: ChartModel,
  axis: { labelColor?: string } | undefined,
  inherited?: string,
): string {
  return axis?.labelColor ?? model.textColor ?? inherited ?? chartLabelDefault(model)
}

function axisTitleColor(
  model: ChartModel,
  axis: { titleColor?: string; labelColor?: string },
  inherited: string,
): string {
  return axis.titleColor ?? model.textColor ?? axis.labelColor ?? inherited
}

function legendTextColor(model: ChartModel): string {
  return (
    model.legendColor ?? model.textColor ?? model.valAxis?.labelColor ?? chartLabelDefault(model)
  )
}

function dataLabelTextColor(model: ChartModel, seriesIndex: number, fallback: string): string {
  return (
    model.series[seriesIndex]?.dataLabelColor ?? model.dataLabelColor ?? model.textColor ?? fallback
  )
}
...
export function buildChartNode(
  ...
  titleLines.forEach((line, i) => {
    node.labels.push({
      ...
      fontSizePx: titleSizePx,
      color: model.titleColor ?? model.textColor ?? (model.hasStylePart ? '#333333' : '#000000'),
      bold: titleBold,
...
function buildChartNodeInner(
  ...
  const labelColor = axisLabelColor(model, model.valAxis)
  const catLabelSizePx = ptToPx(
    model.catAxis?.labelSizePt ?? model.valAxis?.labelSizePt ?? chartTextPt(model),
    vp.scale,
  )
  const catLabelColor = axisLabelColor(model, model.catAxis, labelColor)
  const legendColor = legendTextColor(model)
  ...
  if (sec) {
    const secLabelColor = axisLabelColor(model, model.valAxis2, labelColor)
    node.axisLines.push({
      x1: plot.x + plot.w,
      y1: plot.y,
      x2: plot.x + plot.w,
    ...
    if (model.valAxis2?.title) {
      ...
      node.labels.push({
        ...
        fontSizePx: sz,
        color: axisTitleColor(model, a, secLabelColor),
  ...
  if (model.catAxis?.title) {
    ...
    node.labels.push({
      ...
      fontSizePx: sz,
      color: axisTitleColor(model, a, labelColor),
      ...(a.titleBold ? { bold: true } : {}),
      ...(a.titleItalic ? { italic: true } : {}),
    })
  }
  ...
  const dLbl = (si: number, catIdx: number, cx: number, y: number, v: number, inside: boolean) => {
    ...
    node.labels.push({
      ...
      y,
      fontSizePx: dlSize,
      color: dataLabelTextColor(model, si, inside ? '#FFFFFF' : dlBold ? '#000000' : '#404040'),
      ...(dlBold ? { bold: true } : {}),
    })
  }
  ...
  vals.forEach((v, i) => {
    ...
    if (model.series[0]?.dataLabels ?? model.dataLabels) {
      ...
      node.labels.push({
        ...
        fontSizePx: dlSize,
        color: dataLabelTextColor(model, 0, '#FFFFFF'),
        ...(dlBold ? { bold: true } : {}),
      })
    }
    angle += sweep
...
function buildHBarNode(
  ...
  const dLbl = (
    ...
    node.labels.push({
      ...
      y: yMid - dlSize * 0.55,
      fontSizePx: dlSize,
      color: dataLabelTextColor(model, si, inside ? '#FFFFFF' : '#404040'),
    })
  }
  if (stacked) {
...
function buildScatterNode(
  ...
  xTicksR.ticks.forEach((t) => {
    ...
    node.labels.push({
      ...
      fontSizePx: labelSizePx,
      color: axisLabelColor(model, model.catAxis, labelColor),
    })
  })
  const axisColor = model.valAxis?.lineColor ?? model.catAxis?.lineColor ?? '#888888'
  const axisW = Math.max(1, ptToPx(1, vp.scale))
    ...
    pts.forEach((p) => {
      ...
      if (cellLab) {
        ...
        node.labels.push({
          ...
          color: dataLabelTextColor(model, si, '#404040'),
        })
      }
...
function addSeriesLegend(
  ...
  const labelColor = legendTextColor(model)
  if (legendPos === 't' || legendPos === 'b') {
    const total = itemWs.reduce((a, b) => a + b, 0)
