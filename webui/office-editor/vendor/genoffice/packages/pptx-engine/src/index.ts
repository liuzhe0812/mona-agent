...
export async function savePptx(opened: OpenedPptx): Promise<Uint8Array> {
  return preparePptxSave(opened)()
}

/** Freeze this version synchronously; compress only when its checkpoint is requested. */
export function preparePptxSave(opened: OpenedPptx): () => Promise<Uint8Array> {
  const zip = buildZip(opened)
  let result: Promise<Uint8Array> | undefined
  return () => result ??= zip.generateAsync({
...
export function editChartElement(
  ...
  const textOnly = Object.keys(patch).length > 0 && Object.keys(patch).every((key) =>
    (CHART_TEXT_COLOR_KEYS as readonly string[]).includes(key))
  if (!textOnly && chartEl.descr !== 'aislides-chart') return false
  ...
  const originalXml = archive.readText(chartPath)
  if (!originalXml) return false
  if (textOnly) {
    const updated = patchChartTextColors(originalXml, patch)
    if (updated !== originalXml) {
      archive.entries.set(chartPath, Buffer.from(updated, 'utf8'))
      slide.structureDirty = true
...
export function getChartElementData(
  ...
  textStyle: ChartTextColors
  axisTextStyles: Array<{ labelColor?: string; titleColor?: string }>
  supportedTextStyleFields: readonly string[]
} | null {
  const el = slide.elements.find((e) => matchesElementRef(e, elementId))
  ...
  return {
    ...
    textStyle: {
      textColor: chartEl.chart.textColor,
      titleColor: chartEl.chart.titleColor,
      legendColor: chartEl.chart.legendColor,
      dataLabelColor: chartEl.chart.dataLabelColor,
    },
    axisTextStyles: [chartEl.chart.catAxis, chartEl.chart.valAxis, chartEl.chart.valAxis2]
      .filter((axis) => axis !== undefined)
      .map((axis) => ({ labelColor: axis!.labelColor, titleColor: axis!.titleColor })),
    supportedTextStyleFields: CHART_TEXT_COLOR_KEYS,
  }
}
