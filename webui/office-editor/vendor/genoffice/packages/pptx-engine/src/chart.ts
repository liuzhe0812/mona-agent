...
export interface ChartSeries {
  ...
  dataLabels?: boolean
  /** Explicit per-series data-label text color from c:dLbls/c:txPr */
...
export interface ChartModel {
  ...
  legendPt?: number
  legendBold?: boolean
  /** Explicit legend text color from c:legend/c:txPr */
  legendColor?: string
  ...
  /** Explicit data-label text color from plot-level c:dLbls/c:txPr */
  dataLabelColor?: string
  ...
  /** chartSpace-level <c:txPr> default text color; chart text without its own color uses this */
  textColor?: string
...
export function parseChartXml(
  ...
  const parsePlotSeries = (
    ...
    for (const ser of sers) {
      ...
      const serDlColor = textColorFrom(serDl?.['c:txPr'], undefined, theme)
      if (serDlColor) s.dataLabelColor = serDlColor
  ...
  if (legendNode) {
    ...
    const c = textColorFrom(legendNode['c:txPr'], undefined, theme)
    if (c) model.legendColor = c
  }
  ...
  const chartTxPr = doc['c:chartSpace']?.['c:txPr']
  const txP = chartTxPr?.['a:p']
  ...
  const textColor = textColorFrom(chartTxPr, undefined, theme)
  ...
  if (model.title) {
    ...
    const c = textColorFrom(chart['c:title']?.['c:txPr'], rich, theme)
  ...
  const dLblsInfo = (
    ...
    const dColor = textColorFrom(d['c:txPr'], undefined, theme)
    return {
  ...
  if (found) {
    ...
    const color =
      dLblResults
        .slice(0, cartesian.length > 1 ? cartesian.length : 1)
        .find((r) => r.on && r.color)?.color
    if (color) model.dataLabelColor = color
  }
...
function parseAxis(ax: any, theme?: Theme): ChartAxisStyle | undefined {
  ...
  const labelColor = textColorFrom(ax['c:txPr'], undefined, theme)
  ...
  if (title) {
    ...
    const c = textColorFrom(titleNode?.['c:txPr'], titleNode?.['c:tx']?.['c:rich'], theme)
    if (c) out.titleColor = c
  }
  return Object.keys(out).length ? out : undefined
}

/** Resolve the first explicit text color in a txPr or rich-text run. */
function textColorFrom(txPr: any, rich: any, theme?: Theme): string | undefined {
  const richColor = richTextColor(rich, theme)
  if (richColor) return richColor
  const paras: any[] = Array.isArray(txPr?.['a:p'])
    ? txPr['a:p']
    : txPr?.['a:p']
      ? [txPr['a:p']]
      : []
  for (const p of paras) {
    ...
    for (const run of runs) {
      const c = resolveColorNode(run?.['a:rPr']?.['a:solidFill'], theme)
      if (c) return c
    }
    const c = resolveColorNode(p?.['a:pPr']?.['a:defRPr']?.['a:solidFill'], theme)
    if (c) return c
  }
  return undefined
}

function richTextColor(rich: any, theme?: Theme): string | undefined {
  const paras: any[] = Array.isArray(rich?.['a:p'])
    ? rich['a:p']
    : rich?.['a:p']
      ? [rich['a:p']]
      : []
