...
import { AlertCircle, Clapperboard, Download, FolderOpen, Loader2, Trash2 } from "lucide-react";

import { deleteVideoProject, fetchVideoProjects, buildVideoDownloadUrl, getApiBase, type VideoProject, type VideoProjectPhase } from "@/lib/api";
...
export function VideoHistory({ currentProjectName, collapsed = false, refreshKey = 0, onSelect, onDelete }: VideoHistoryProps) {
  ...
  const metaLine = (p: VideoProject) =>
    `${VIDEO_PHASE_LABELS[p.phase] ?? p.phase} · ${formatResolution(p.resolution)}${p.outputStale ? " · 内容已变化" : ""}`;
            ...
            return (
              <ContextMenu key={p.name}>
                ...
                <ContextMenuContent className="w-40">
                  {p.hasVideo && (
                    <ContextMenuItem onClick={() => void handleDownload(p.name)} className="text-[12px]">
                      <Download className="mr-2 h-3.5 w-3.5" />
                      下载 MP4
                    </ContextMenuItem>
                  )}
                  <ContextMenuItem onClick={() => void handleOpenDir(p.name)} className="text-[12px]">
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                将删除项目「{pendingDelete}」及其全部分镜、预览和导出文件，删除后无法恢复。
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
