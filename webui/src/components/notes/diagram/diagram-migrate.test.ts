...
import { FLOWCHART_NODE_KINDS } from "../flowchart/flowchart-document";
import {
  V1_NODE_KIND_MAP,
...
function v1Doc(nodes: FlowchartNode[], edges: FlowchartEdge[] = []): FlowchartDocument {
  return { version: 1, direction: "TB", nodes, edges };
}

describe("V1_NODE_KIND_MAP", () => {
  it("覆盖 v1 全部节点 kind", () => {
    for (const kind of FLOWCHART_NODE_KINDS) {
      expect(V1_NODE_KIND_MAP[kind], `缺少映射：${kind}`).toBeDefined();
    }
  });
