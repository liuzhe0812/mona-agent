...
import { isNamedCategory } from "./profile-categories";
...
function TrendPlot({ trends, labels }: { trends: Array<{ topic: string; points: number[] }>; labels: string[] }) {
  const width = 700; const height = 220; const max = Math.max(...trends.flatMap((item) => item.points), 1); const colors = trends.map((trend) => profileChartColor(trend.topic));
  return <div className="flex min-h-0 flex-1 flex-col"><svg viewBox={`0 0 ${width} ${height}`} className="min-h-[210px] flex-1 w-full" aria-label="话题变化趋势"><line x1="42" y1="175" x2="680" y2="175" className="stroke-border" /><line x1="42" y1="24" x2="42" y2="175" className="stroke-border" />{trends.slice(0, 3).map((trend, trendIndex) => { const step = trend.points.length > 1 ? 638 / (trend.points.length - 1) : 638; const path = trend.points.map((point, index) => `${index === 0 ? "M" : "L"}${42 + step * index},${175 - (point / max) * 135}`).join(" "); return <g key={trend.topic}><path d={path} fill="none" stroke={colors[trendIndex % colors.length]} strokeWidth="2" strokeDasharray={[undefined, "6 3", "2 3"][trendIndex]} />{trend.points.map((point, index) => <circle key={`${trend.topic}-${index}`} cx={42 + step * index} cy={175 - (point / max) * 135} r="2.5" fill={colors[trendIndex % colors.length]} />)}</g>; })}{labels.slice(0, 6).map((label, index) => <text key={`${label}-${index}`} x={42 + (labels.length > 1 ? (638 / (labels.length - 1)) * index : 319)} y="205" textAnchor="middle" className="fill-muted-foreground text-micro">{label}</text>)}</svg><div className="flex flex-wrap gap-4 text-micro text-muted-foreground">{trends.slice(0, 3).map((trend, index) => <span key={trend.topic} className="flex items-center gap-1"><i className="w-4 border-t-2" style={{ borderColor: colors[index % colors.length], borderStyle: (["solid", "dashed", "dotted"] as const)[index] }} />{trend.topic}</span>)}</div></div>;
}

function ComparisonBars({ topics }: { topics: Array<{ topic: string; count: number; previous_count?: number; delta?: number }> }) { const max = Math.max(...topics.map((item) => Math.max(item.count, item.previous_count ?? 0)), 1); return <div className="space-y-4 py-2">{topics.slice(0, 4).map((item) => <div key={item.topic}><div className="mb-1.5 flex justify-between gap-2 text-caption"><span>{item.topic}</span><span className="text-muted-foreground">{item.previous_count ?? "—"} → {item.count}</span></div><div className="flex gap-1.5"><div className="h-2 flex-1 bg-muted"><div className="h-full bg-muted-foreground/40" style={{ width: `${((item.previous_count ?? 0) / max) * 100}%` }} /></div><div className="h-2 flex-1"><div className="h-full" style={{ width: `${(item.count / max) * 100}%`, background: profileChartColor(item.topic) }} /></div></div></div>)}</div>; }
...
function ActivityCalendar({ daily, start, end }: { daily: Array<{ date: string; user_messages: number }>; start: string; end: string }) {
  ...
  return <div className="mx-auto w-fit"><div className="mb-1 grid grid-cols-7 gap-1 text-center text-micro text-muted-foreground">{"一二三四五六日".split("").map((day) => <span key={day} className="w-3">{day}</span>)}</div><div className="grid grid-cols-7 gap-1">{Array.from({ length: firstWeekday }, (_, index) => <span key={`blank-${index}`} className="h-3 w-3" />)}{days.map((item) => <span key={item.date} title={`${item.date} · ${item.user_messages} 条`} className="h-3 w-3 bg-muted" style={item.user_messages > 0 ? { background: profileChartColorAlpha(PROFILE_CHART_COLORS.teal, 0.2 + (item.user_messages / max) * 0.68) } : undefined} />)}</div></div>;
}

function normalizeTopicComparison(raw: unknown): Array<{ topic: string; count: number; previous_count?: number; delta?: number }> { if (!Array.isArray(raw)) return []; return raw.map((item) => item as { topic?: string; current?: number; count?: number; previous?: number; previous_count?: number; delta?: number }).map((item) => ({ topic: item.topic ?? "", count: Number(item.current ?? item.count ?? 0), previous_count: item.previous ?? item.previous_count, delta: item.delta })).filter((item) => isNamedCategory(item.topic)); }
function normalizeTopicTrends(raw: ProfileCharts["topic_trends"] | undefined): Array<{ topic: string; points: number[] }> { return raw?.series?.filter((item) => isNamedCategory(item.topic)).map((item) => ({ topic: item.topic, points: item.values.filter((value) => Number.isFinite(value)) })) ?? []; }
function normalizeNewTopics(raw: ProfileCharts["new_topics"] | undefined) { return raw?.filter((item) => isNamedCategory(item.topic)) ?? []; }
function normalizeDimensions(raw: ProfileCharts["profile_dimensions"] | undefined): Array<{ axis: string; value: number }> { return raw?.filter((item) => isNamedCategory(item.axis)).map((item) => ({ axis: item.axis, value: item.count })) ?? []; }
