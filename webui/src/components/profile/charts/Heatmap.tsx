...
export function ActivityHeatmap({ data }: HeatmapProps) {
  ...
  return (
    <div className="inline-flex flex-col gap-1 overflow-visible">
        {/* 小时标签 */}
        <div className="flex gap-0.5 pl-10 text-micro text-muted-foreground">
        ...
        {data.map((row, day) => (
          <div key={day} className="flex items-center gap-1">
            <span className="w-10 shrink-0 text-micro text-muted-foreground">{DAYS[day]}</span>
        ...
        {/* 图例 */}
        <div className="mt-2 flex items-center justify-end gap-1.5 text-micro text-muted-foreground">
          <span>少</span>
