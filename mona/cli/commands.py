...
# Remove default handler and re-add with unified mona format
logger.remove()
...
from mona import __logo__, __version__
...
app = typer.Typer(
    ...
    help=f"{__logo__} mona - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from mona.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} mona[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} mona[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} mona v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize mona configuration and workspace."""
    from mona.config.loader import get_config_path, load_config, save_config, set_config_path
    from mona.config.schema import Config
    ...
    if wizard:
        from mona.cli.onboard import run_onboard
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/mona#-chat-apps[/dim]"
    )
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from mona.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from mona.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show mona runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'mona-ai[api]'[/red]")
        raise typer.Exit(1)
...
def _run_gateway(
    ...
    *,
    ...
    from mona.channels.websocket import publish_runtime_model_update
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show mona runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from mona.channels.registry import discover_all
    from mona.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from mona.channels.registry import discover_all, discover_channel_names
...
def status():
    ...
    console.print(f"{__logo__} mona Status\n")
...
def _login_github_copilot() -> None:
    try:
        from mona.providers.github_copilot_provider import login_github_copilot
