...
- `_manifest.json` — generation manifest (status=Pending), runnable via `generate_image` tool
...
To reproduce or regenerate, use Mona's `generate_image` tool for each item in the manifest:

```
# For each subdirectory (rendering / palette / type), invoke generate_image
# for each item in the corresponding _manifest.json with status: Pending
```
