...
> 4. Search for each concept's filename in the chosen library: `ls skills/mona-ppt/templates/icons/<chosen-library>/ | grep <keyword>`
...
python3 skills/mona-ppt/scripts/latex_render.py <project_path>
```
...
**When recommending C** — surface its two implementation modes so the user knows "no API key" is a supported state:

| Mode | Trigger | Mechanism |
|---|---|---|
| **Path A** | Host has a native image tool (Mona's `generate_image` / Codex / Antigravity / Claude Code / similar) — auto-selected, no user prompting needed | Host-native generation via `generate_image` tool |
| **Offline Manual** | Host has no native image tool | Prompts written to `images/image_prompts.json`; user generates externally and places files in `project/images/` |

Selection is automatic in Step 5 (A → Manual). Detailed contract: [`image-generator.md`](./image-generator.md) §7.
...
### Smart Chart Selection Rules (MANDATORY)

After content planning, evaluate each page against the following conditions. If any condition is met, the corresponding architecture diagram MUST be auto-generated — no manual specification needed:

#### Trigger Conditions & Layout Mapping

| Content Characteristic | Auto-trigger | Layout Method |
|------------------------|-------------|---------------|
| ≥3 hierarchical levels with upstream-downstream support/dependency (e.g. "infrastructure → platform → application") | Layered Architecture Diagram | Bottom-up layers, each layer in an independent color block |
| End-to-end process with ≥3 sequential stages (e.g. "collect → process → analyze → apply") | Flow Diagram | Left-to-right, stages connected by arrows |
| Core entity at center with surrounding parallel modules (e.g. "platform + 6 capabilities", "ecosystem + 3 pillars") | Radiating Architecture Diagram | Core centered, modules orbiting around it |
| ≥2 objects in parallel comparison with similar structure but different dimensions (e.g. "Plan A vs Plan B", "2024 vs 2025") | Comparison Architecture Diagram | Side-by-side columns, same color family per side |
| Cross-combination relationships among multiple elements (e.g. "technology × scenario" matrix, "capability × industry" mapping) | Matrix Architecture Diagram | Tabular layout, rows and columns as two dimensions |

#### Drawing Specification

- Each layer/stage uses an independent color block; sub-modules within the same layer use lighter shades of the same color family
- Arrows are unified gray unidirectional solid lines, annotated with relationship type (support / call / flow / dependency)
- Layer names or stage names are labeled on the left side or top
- Bottom-right corner: data source attribution (e.g. "Data source: CAICT《Report Name》")

#### Priority Rule

**Architecture diagram > Data chart > Text bullets.** When a page's content satisfies both data visualization (bar/line/pie) and architecture diagram conditions simultaneously, the architecture diagram takes priority; data is embedded as cards within the corresponding architecture modules.

#### Implementation Notes

- **Data charts** (bar, line, pie, scatter, radar, etc.): use `templates/charts/<chart_name>.svg` as baseline — adapt project colors/typography/data; embed `<!-- chart-plot-area -->` marker for `verify-charts` calibration
- **Architecture diagrams** (layered, flow, radiating, comparison, matrix): draw directly in SVG using `<rect>` / `<line>` / `<text>` / `<path>` elements; reference `templates/charts/` for layout patterns
- **Complex charts** (sankey, heatmap, box plot, etc.): draw from scratch in SVG following `design_spec.md §VII` guidance; embed chart-plot-area marker for coordinate verification
...
> **Principle — proportion follows information weight, not preset ratios.** Combine patterns, break the grid for `breathing` pages, or propose new patterns.

### 4.1 Content-Driven Layout Decision Rules (MANDATORY)

When planning each page's layout, it is FORBIDDEN to use the same layout pattern for all pages.
You MUST select the best-matching layout from the table below based on that page's Core Message content type:

| Content Type | Matching Layout | Structure Requirements |
|-------------|----------------|----------------------|
| Data comparison (≥2 quantifiable groups) | Asymmetric split — Chart + Bullets | Left 60% chart, right 40% 3-5 bullet points |
| Timeline / evolution (≥3 time nodes) | Top-bottom split — Timeline | Centered horizontal axis, circular node markers, events staggered above/below |
| Parallel modules / capability matrix (≥3 peer modules) | Four-quadrant / matrix — Card Grid | 2×2 or 3×2 uniform grid, each card: icon + title + short description |
| Process / pipeline (sequential dependencies) | Three-column — Flow | Left-to-right arrows connecting stages, each stage block: name + key points |
| Layered architecture (top-down support) | Top-bottom split — Stacked Layers | Bottom-up color blocks, arrows marking support relationships |
| Comparative analysis (A vs B) | Symmetric split (5:5) — Split Panel | Left column A, right column B, center divider, same-dimension alignment |
| Deep dive on one concept | Asymmetric split — Text + Visual | Left 50% text points + right 50% diagram / architecture |
| Summary / conclusion | Single column centered — Bullet Cascade | Points top-to-bottom, each with left number circle + bold keyword |

### 4.2 Layout Diversity Constraint

- Any single layout pattern MUST NOT appear more than **3 times** across the entire deck
- Cover, table of contents, and closing/thank-you pages are exempt from this constraint
- If the page count requires exceeding this limit, vary the layout by combining patterns (e.g., Card Grid + Asymmetric split) or switching to a visually distinct alternative

### 4.3 Layout Pattern Reference
...
### 4.4 Layout Field in Content Outline

Each page's Layout field in the Content Outline (§IX) MUST:
1. Select a layout name from §4.3 Pattern column
2. Include a one-line explanation of why this content type matches this layout (per §4.1 decision rules)
3. Respect the diversity constraint in §4.2
...
3. Layout patterns — combine, nest, or break (§4.3 lists 11 patterns as reference, not exhaustive); content-to-layout mapping is mandatory per §4.1; diversity constraint per §4.2
...
5. **Generate page visual plan**: After `spec_lock.md`, write `ppt_projects/<project_name>/page_visual_plan.json` — a structured JSON file recording each page's chosen visual expression. This file enables the frontend to preview the visual plan before generation starts.
   ...
   **visual_type** must be one of: `text_layout` | `data_chart` | `flowchart` | `architecture` | `timeline` | `matrix` | `comparison` | `ai_image` | `mixed`.

   - `data_chart`: page uses a chart template from `charts_index.json` → set `chart_template` to the key.
   - `flowchart` / `architecture` / `timeline` / `matrix` / `comparison`: structured diagram pages → `chart_template` is null, describe in `notes`.
   - `ai_image`: page relies primarily on an AI-generated image → set `has_ai_image: true`.
   - `text_layout`: pure text/number layout, no chart or diagram.
   - `mixed`: combines multiple visual types → describe in `notes`.

   Every page in §IX Content Outline MUST appear exactly once.
...
```bash
python3 scripts/project_manager.py init <project_name> --format <canvas_format> --dir ppt_projects
```

Save outputs to `ppt_projects/<project_name>/design_spec.md`.
