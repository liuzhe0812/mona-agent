...
|---|---|---|---|
| `ai` | [`image-generator.md`](./image-generator.md) | `generate_image` tool | `Generated` |
...
`Needs-Manual` is also the entry status for **Offline Manual Mode** (no host-native image tool in use). Affected ai rows are marked `Needs-Manual` from the start without a failed attempt — see [`image-generator.md`](./image-generator.md) §7.
...
Executor does NOT invoke `generate_image` / `image_search.py`.
