...
**Execution trace (Mandatory)**: After each re-read, output a one-line confirmation before writing the SVG. Format:

```
📋 P<NN>: spec_lock re-read — colors: <primary/secondary/accent HEX> — fonts: <title_family / body_family> — rhythm: <page_rhythm tag>
```

This trace line proves the re-read happened and makes drift detectable in conversation. Omitting it = rule violation.
