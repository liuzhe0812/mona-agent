...
Role definition for the **AI image generation path**: convert each `Acquire Via: ai` row into an optimized prompt, generate the image via Mona's `generate_image` tool, and save it to `project/images/`.
...
Write `project/images/image_prompts.json` per §6. This manifest records all prompts and metadata for the deck's AI images. It serves as the source of truth for generation status and is consumed by the generation step.
...
| `items[].aspect_ratio` | yes | Container sizing | Passed to `generate_image` tool |
...
| `items[].status` | yes | Generation manages | `Pending` initially; updated to `Generated` / `Failed` / `Needs-Manual` |
...
C (AI-generated) supports two implementation modes sharing one `image_prompts.json` source:

| Trigger | Mode | Mechanism |
|---|---|---|
| **Default** — host has a native image tool | **Path A**: `generate_image` tool | Agent invokes Mona's `generate_image` tool for each item; outputs land at `project/images/<filename>` |
| **Path A unavailable** | **Offline Manual Mode** | Manifest stays on disk; user generates externally from `items[].prompt` and places files at `project/images/<filename>` |

**Selection logic** — monotonic A → B fallback chain (automatic, no user prompting):

1. **Try Path A** — if the host has a native image tool (Mona's `generate_image` / Codex / Antigravity / Claude Code / similar), invoke it directly for each item in the manifest.
2. **Fall to Offline Manual** — if no host-native tool is available, write prompts to `images/image_prompts.json` and hand off to the user.

**User override**: If the user explicitly names a tool ("use Codex's image tool"), use that tool. Explicit naming is the only way to override the default; otherwise the chain is monotonic.
...
### Path A — `generate_image` tool (Default)

For each item in `image_prompts.json` with `status: Pending`:

1. Call Mona's `generate_image` tool with the item's `prompt` and `aspect_ratio`
2. Save the generated image to `project/images/<filename>`
3. Update the item's `status` to `Generated` in `image_prompts.json`
4. If generation fails, retry once; on second failure, set `status: Needs-Manual`
...
generate_image(
    prompt=<items[].prompt>,
    aspect_ratio=<items[].aspect_ratio>,
...
- **Sequential by default**: process items one at a time to avoid rate limits and ensure each image is saved correctly
- **Idempotent**: re-running only re-processes entries whose status is `Pending` or `Failed`
- **No external configuration needed**: the `generate_image` tool uses the host's built-in image generation capability — no `IMAGE_BACKEND`, API keys, or `.env` configuration required

### Offline Manual Mode (Fallback)

**Trigger**: No host-native image tool is available.
...
4. Print one consolidated handoff to the user:
   ...
   - Pointer to `images/image_prompts.json` (`items[].prompt`)
...
If `generate_image` fails twice in a row for an item:

1. Do not halt. Mark the row `Needs-Manual`.
...
2. Re-invoke `generate_image` for that item
