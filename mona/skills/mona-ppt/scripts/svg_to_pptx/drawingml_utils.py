...
from .drawingml_context import IDENTITY_MATRIX, AffineMatrix, ConvertContext
...
def parse_hex_color(color_str: str) -> str | None:
    """Parse '#RRGGBB', '#RGB', 'rgb(R,G,B)', or 'rgba(R,G,B,A)' to 'RRGGBB'.

    For rgba() format, the alpha component is NOT returned here — callers that
    need alpha should use ``parse_color_with_alpha`` instead.
...
def parse_color_with_alpha(color_str: str) -> tuple[str | None, float]:
    """Parse a color string and return (hex_color, alpha).
...
def parse_stop_style(style_str: str) -> tuple[str | None, float]:
    ...
    Returns:
        (color, opacity) tuple.  Color may carry an embedded alpha from rgba()
        — in that case the returned opacity already incorporates it.
    """
    color = None
    opacity = 1.0
    if not style_str:
