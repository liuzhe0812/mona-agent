...
def convert_circle(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if _is_donut_circle(elem, ctx):
        ...
        if grad_id and grad_id in ctx.defs:
            ...
            color, c_alpha = parse_color_with_alpha(stroke_val)
            eff_op = op
            if c_alpha < 1.0:
                eff_op = (op or 1.0) * c_alpha
            fill = build_solid_fill(color, eff_op) if color else '<a:noFill/>'
        else:
            fill = '<a:noFill/>'
...
def _override_run_attrs(
    ...
    if tspan.get('fill'):
        ...
        c, c_alpha = parse_color_with_alpha(child_fill)
        if c:
            run_attrs['fill'] = c
            if c_alpha < 1.0:
                run_attrs['fill_alpha'] = c_alpha
    if tspan.get('stroke'):
...
def _build_text_outline_xml(run: dict[str, Any]) -> str:
    ...
    color, c_alpha = parse_color_with_alpha(stroke_raw)
    if not color:
        return ''
...
def _build_run_xml(
    ...
    fill_alpha = run.get('fill_alpha')
    if fill_alpha is not None and fill_alpha < 1.0:
        opacity = (opacity or 1.0) * fill_alpha
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    fill_color, fill_alpha = parse_color_with_alpha(fill_raw)
