...
def build_gradient_fill(
    ...
    for child in grad_elem:
        ...
        if not color:
            raw = child.get('stop-color', '#000000')
            color, c_alpha = parse_color_with_alpha(raw)
            if c_alpha < 1.0:
                stop_opacity *= c_alpha
        if color is None:
            color = '000000'
...
def build_fill_xml(
    ...
    color, c_alpha = parse_color_with_alpha(fill)
    if color:
        effective_opacity = opacity
        if c_alpha < 1.0:
            effective_opacity = (opacity or 1.0) * c_alpha
...
def build_pattern_fill(
    ...
    fg_hex, fg_alpha = parse_color_with_alpha(fg_color) if fg_color else (None, 1.0)
    bg_hex, _ = parse_color_with_alpha(bg_color) if bg_color else (None, 1.0)
    if not fg_hex:
        return ''
    ...
    eff_opacity = opacity
    if fg_alpha < 1.0:
        eff_opacity = (opacity or 1.0) * fg_alpha
    if eff_opacity is not None and eff_opacity < 1.0:
        alpha_xml = f'<a:alpha val="{int(eff_opacity * 100000)}"/>'
...
def build_stroke_xml(
    ...
    # Solid color stroke
    color, c_alpha = parse_color_with_alpha(stroke)
    if not color:
        return '<a:ln><a:noFill/></a:ln>'

    eff_opacity = opacity
    if c_alpha < 1.0:
        eff_opacity = (opacity or 1.0) * c_alpha
    alpha_xml = ''
    if eff_opacity is not None and eff_opacity < 1.0:
        alpha_xml = f'<a:alpha val="{int(eff_opacity * 100000)}"/>'
