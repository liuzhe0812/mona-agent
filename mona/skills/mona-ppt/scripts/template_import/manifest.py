...
def detect_background_asset(root: ET.Element | None, rels: dict[str, dict[str, str]]) -> str | None:
    ...
    # 1. Check <p:bg> element (official background)
    ...
    # 2. Check for full-page shapes with image fills (common in Chinese templates)
    # A shape covering >80% of the slide area with an image fill is likely a background
    sp_tree = root.find("p:cSld/p:spTree", NS)
    if sp_tree is None:
        return None

    for elem in sp_tree:
        # 2a. Check <p:sp> (shape with image fill)
        if elem.tag == f"{{{NS['p']}}}sp":
            ...
            # Skip shapes that don't cover most of the slide
            # Standard 16:9 slide is 12192000 x 6858000 EMU
            if x > 500000 or y > 500000:
                continue
            if w < 10000000 or h < 5000000:
                continue

            # Check for image fill (blipFill)
            blip = elem.find("p:spPr/a:blipFill/a:blip", NS)
            if blip is None:
                blip = elem.find(".//a:blipFill/a:blip", NS)
            if blip is None:
                continue
        ...
        # 2b. Check <p:pic> (picture element covering most of the slide)
        if elem.tag == f"{{{NS['p']}}}pic":
            xfrm = elem.find("p:spPr/a:xfrm", NS)
            if xfrm is None:
                continue
            off = xfrm.find("a:off", NS)
