...
When the output format **is** explicitly specified (PDF, DOCX, Lark doc, Markdown, webpage), adapt formatting conventions accordingly and delegate format-specific rendering to the appropriate sub-Skill or tool Skill (e.g., `pdf`, `docx`). For presentations, use the `mona-ppt` skill.
