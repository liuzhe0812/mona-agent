---
name: update-setup
description: One-time setup wizard for the mona upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `mona --version`. Tell the user the current version.
...
```
command -v mona || true
python -m pip show mona-ai || true
pipx list | sed -n '/mona-ai/,+3p' || true
uv tool list | sed -n '/mona-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how mona was
installed.
...
`question: "Where is your mona source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "mona-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "mona-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "mona-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade mona to the latest version. Triggers: upgrade mona, update mona, 升级mona, 更新mona."
---
...
4. Use `exec` to verify: `mona --version`
5. Tell the user the new version. Say: "Run `/restart` to restart mona and apply the update. If `/restart` is unavailable in this channel, restart the mona process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade mona' when you want to update."
