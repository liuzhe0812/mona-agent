...
## Mona Environment Notes

### Skill Directory Resolution

When running in Mona, `${SKILL_DIR}` resolves to the skill installation directory:
- Builtin skill: `<mona-install-dir>/mona/skills/ppt-master/`
- Workspace skill: `<workspace>/skills/ppt-master/`

All `${SKILL_DIR}` references in commands should be resolved to the actual skill directory path.
...
**Fallback**: If `generate_image` is not available, fall back to:
```bash
python3 ${SKILL_DIR}/scripts/image_gen.py "prompt" --backend openai
```
...
| Index | Path | Purpose |
|-------|------|---------|
| Layout templates | `${SKILL_DIR}/scripts/templates_full/layouts/layouts_index.json` | Query available page layout templates |
| Brand presets | `${SKILL_DIR}/scripts/templates_full/brands/brands_index.json` | Query available brand identity presets (color / typography / logo / voice) |
| Visualization templates | `${SKILL_DIR}/scripts/templates_full/charts/charts_index.json` | Query available visualization SVG templates (charts, infographics, diagrams, frameworks) |
| Icon library | `${SKILL_DIR}/scripts/templates_full/icons/` | See `${SKILL_DIR}/scripts/templates_full/icons/README.md`; search icons on demand with `ls scripts/templates_full/icons/<library>/ \| grep <keyword>` |

## Standalone Workflows

| Workflow | Path | Purpose |
|----------|------|---------|
| `topic-research` | `references/workflows/topic-research.md` | Pre-pipeline — gather web sources when the user supplies only a topic with no source files |
| `create-template` | `references/workflows/create-template.md` | Standalone layout template creation workflow |
| `create-brand` | `references/workflows/create-brand.md` | Standalone brand-only template creation (identity preset; no SVG page roster) |
| `resume-execute` | `references/workflows/resume-execute.md` | Phase B entry — resume execution in a fresh chat after Phase A (Step 1–5) completed in another session (split mode) |
| `verify-charts` | `references/workflows/verify-charts.md` | Chart coordinate calibration — run after SVG generation if the deck contains data charts |
| `customize-animations` | `references/workflows/customize-animations.md` | Object-level PPTX animation customization — run only when the user explicitly asks to tune animation order/effects/timing |
| `live-preview` | `references/workflows/live-preview.md` | Browser-based live preview — auto-started during generation and re-enterable any time the user mentions "live preview", "preview", "看效果", or wants to click/select a slide element |
| `visual-review` | `references/workflows/visual-review.md` | Per-page rubric-based visual self-check — run only when the user explicitly asks for a visual re-pass on the generated SVGs (between Executor and post-processing). Opt-in only; never invoked by the main pipeline. |
...
> **No source content?** When the user supplies only a topic name or requirements without any file or substantive description, run the [`topic-research`](references/workflows/topic-research.md) workflow first, then return here with its products as input.
...
> To create a new layout or deck, read [`references/workflows/create-template.md`](references/workflows/create-template.md). To create a new brand, read [`references/workflows/create-brand.md`](references/workflows/create-brand.md).
...
|---|---|
| Heavy (long page count / bulky sources / heavy web-fetch accumulation) | State estimated page count and large source size; recommend switching to [split mode](references/workflows/resume-execute.md) after Step 5 — stop this chat, open a fresh window and input `继续生成 projects/<project_name>` to enter Phase B (SVG generation + export); no response or "continue" = default continuous mode. |
| Normal (default) | State scale is moderate, default continuous mode generates in one go; if mid-way window switch is desired, input `继续生成 projects/<project_name>` after Step 5 to switch to [split mode](references/workflows/resume-execute.md). |
...
**Default — auto-proceed to Step 6.** Only when the user's Step 4 response explicitly opted into split mode (in reply to the optional hint), output the Phase A hand-off below and stop this conversation:
  ...
  - [ ] **Next**: open a fresh chat window and input `继续生成 projects/<project_name>` to enter Phase B via the [`resume-execute`](references/workflows/resume-execute.md) workflow.
  ```
...
- **Do NOT read or apply submitted annotations during generation.** Users may annotate at any time, but Executor proceeds without touching them. The window to apply annotations opens only after Step 7 completes — see [`references/workflows/live-preview.md`](references/workflows/live-preview.md).
- UI button semantics and editor details: see [`references/workflows/live-preview.md`](references/workflows/live-preview.md) Notes.
...
Run the standalone [`customize-animations`](references/workflows/customize-animations.md) workflow. Default export already has global entrance animation; do not create `animations.json` unless object-level customization was requested.

**Optional recorded narration** (only when the user asks for narrated/video export):

Run the standalone [`generate-audio`](references/workflows/generate-audio.md) workflow. The AI picks a narration backend (`edge` by default, or a configured cloud provider such as ElevenLabs / MiniMax / Qwen / CosyVoice for high-quality or cloned voices), asks the user once (backend + voice + rate/settings + embed-or-not, all with recommended values), then executes `notes_to_audio.py` and (if chosen) re-exports the PPTX with `--recorded-narration audio`.
...
> **Post-export annotation window**: the preview service from Step 6 typically remains running after export. If the user submitted annotations in the browser (during Executor or after export) and now asks to apply them — they may quote the browser prompt (`Annotations saved. ... apply my annotations`), say "apply my annotations" / "应用注解" / equivalent — run [`live-preview`](references/workflows/live-preview.md) Step 2 to apply and re-export. Annotations submitted during generation are also handled here, not earlier.

> **Preview not running?** Any time the user mentions "live preview", "preview", "看效果", or wants to select/click a slide element and the service is not running, run [`live-preview`](references/workflows/live-preview.md) Step 1 to start it. If the service is already running, just point them at the URL — do not restart.
