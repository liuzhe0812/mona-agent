...
3. 用最新 `inspect` 或上一次 `apply` 返回的 `version` 填入 `expected_version`，以章节、表格或格式区域为单位分批 `apply`，让用户能看到有意义的进展；每批操作总数不超过 50。文档含图片时按阅读顺序安排，图片未准备好先提交文本/结构，不等待整套图片或假设存在后台生图 API。已知操作字段直接使用；只有新操作才调用 `inspect capabilities`，并指定具体操作名（例如 `operations: ["set_table_cell"]`），再按需读取对应参考。
4. 每批使用回执继续；需要结构或版式验收时，用 `blocks` 检查受影响块，用 `visual` 检查当前 viewport 或指定的零基 `pageIndex`。图像响应也带 `version`；若发生冲突，重新读取相关结构和画面，保留用户刚做的修改，再提交调整后的操作，不重放旧版本。
5. 会话断开或暂时失败时，用 `office` 的 `open` 携带原 `session_id` 恢复同一会话；不要新建会话或重建文档。完成后按请求调用 `save` 或 `export`。保存和导出都使用最新版本；导出 `output` 必须是工作区相对路径。

## 可用操作

外部材料先用适合其格式的读取工具获取一次；目标编辑器的选区不等于原材料。回执明确提示截断、分页或全文另存时，按缺失范围或全文引用补读，不切换工具重复读取已完整取得的内容。

当前 Docs 编辑器支持 `replace_block_text`、`delete_block`、`insert_paragraph`、`insert_heading`、`insert_list`、`insert_table`、`set_table_cell`、`insert_image`、`set_block_style`、`set_table_style`、`set_page_style` 和 `set_header_footer`。用稳定块 ID 定位内容；表格的 `rowIndex`、`columnIndex` 从 0 开始。`set_page_style` 的页面尺寸和页边距使用毫米，`set_table_style` 的列宽和内边距使用像素。默认/最终节的页眉页脚和页码有实时往返验证；首节、偶数页和复杂多节变体不要宣称完全支持。详细字段和完整端到端 JSON 见 [office-example.md](references/office-example.md)。
