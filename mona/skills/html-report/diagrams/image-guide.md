...
Calls the `generate_image` tool (Mona's built-in image generation) to generate illustrations, backgrounds, hero images, and conceptual visual assets, then base64-encodes and inlines into HTML.
...
3. **Call `generate_image` one by one**, saving as `{name}_{width}x{height}.png`
