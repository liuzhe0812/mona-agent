...
from mona.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("mona_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply mona attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _handle_error(
        ...
        *,
        ...
        upstream_message = LLMProvider._extract_error_message(body)
        if upstream_message:
            msg = f"Error: {upstream_message}"
    ...
    async def chat_stream(
        ...
        await self._ensure_client()
        idle_timeout_s = int(os.environ.get("mona_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
