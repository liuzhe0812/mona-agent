...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~mona.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when a sustained goal is
    active; ``None`` means use ``mona_LLM_TIMEOUT_S``. Pass in-memory ``metadata`` when the
    caller already holds :attr:`~mona.session.manager.Session.metadata` for this turn.
    """
