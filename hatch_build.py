"""Hatch build hook that bundles the webui (Vite) into mona/web/dist.
...
- No-op when `webui/package.json` is absent (e.g. installing from an sdist that
  already contains a prebuilt `mona/web/dist/`).
- Skips when `mona_SKIP_WEBUI_BUILD=1` is set.
- Skips when `mona/web/dist/index.html` already exists, unless
  `mona_FORCE_WEBUI_BUILD=1` is set.
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        if os.environ.get("mona_SKIP_WEBUI_BUILD") == "1":
            self.app.display_info("[webui-build] skipped via mona_SKIP_WEBUI_BUILD=1")
            return

        if not package_json.is_file():
            self.app.display_info(
                "[webui-build] no webui/ source tree, assuming prebuilt mona/web/dist/"
            )
            return

        force = os.environ.get("mona_FORCE_WEBUI_BUILD") == "1"
        if index_html.is_file() and not force:
            self.app.display_info(
                f"[webui-build] reusing existing build at {dist_dir} "
                "(set mona_FORCE_WEBUI_BUILD=1 to rebuild)"
            )
            return
        ...
        if runner is None:
            raise RuntimeError(
                ...
                "install one or set mona_SKIP_WEBUI_BUILD=1 to bypass."
            )
