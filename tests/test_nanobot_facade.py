"""Tests for the mona programmatic facade."""
...
from mona.mona import mona, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        mona.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = mona.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("mona.config.loader.load_config") as mock_load, \
         patch("mona.providers.factory.make_provider") as mock_prov:
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from mona.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = mona.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert mona.mona is mona
    assert mona.RunResult is RunResult
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from mona.agent.hook import AgentHookContext
    from mona.bus.events import OutboundMessage
