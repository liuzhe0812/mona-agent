...
SCRIPT_DIR = Path("mona/skills/skill-creator/scripts").resolve()
...
def test_validate_skill_accepts_existing_skill_creator() -> None:
    valid, message = quick_validate.validate_skill(Path("mona/skills/skill-creator").resolve())
...
def test_validate_skill_accepts_standard_dependency_manifests(tmp_path: Path) -> None:
    ...
    (skill_dir / "package.json").write_text(
        '{"name":"example-skill","private":true}\n',
        encoding="utf-8",
    )
