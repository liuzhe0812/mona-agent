...
def test_webui_title_update_uses_captured_llm_runtime(
    ...
    monkeypatch.setattr(
        "mona.session.webui_turns.maybe_generate_webui_title_after_turn",
...
def test_save_turn_attaches_task_plan_to_assistant_history() -> None:
    loop = _mk_loop()
    session = Session(key="websocket:task-plan-history")
    session.metadata[TASK_PLAN_KEY] = {
        "task_id": "task-1",
        "revision": 2,
        "steps": [{"id": "one", "step": "Deliver", "status": "completed"}],
    }
    loop._save_turn(session, [{"role": "assistant", "content": "Done"}], skip=0)
    assert session.messages[0]["task_plan"]["steps"][0]["step"] == "Deliver"
...
def test_save_turn_stamps_task_and_token_usage_on_assistant() -> None:
    loop = _mk_loop()
    session = Session(key="websocket:usage")
    ...
    assert session.messages[-1]["task_id"] == "task-usage"
    assert session.messages[-1]["token_usage"]["total_tokens"] == 125
...
@pytest.mark.asyncio
async def test_process_message_persists_profile_advice_origin(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    loop._run_agent_loop = AsyncMock(side_effect=RuntimeError("boom"))  # type: ignore[method-assign]
    msg = InboundMessage(
        channel="websocket",
        sender_id="u1",
        chat_id="profile-advice",
        content="开始建议",
        metadata={"origin": "profile_advice", "profile_advice_id": "advice-1"},
    )

    with pytest.raises(RuntimeError, match="boom"):
        await loop._process_message(msg)

    loop.sessions.invalidate("websocket:profile-advice")
    persisted = loop.sessions.get_or_create("websocket:profile-advice")
    assert persisted.messages[0]["origin"] == "profile_advice"
    assert persisted.messages[0]["profile_advice_id"] == "advice-1"
...
@pytest.mark.asyncio
async def test_mona_direct_mention_scope_is_ephemeral_and_resets_next_turn(
    tmp_path: Path,
) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    session = loop.sessions.get_or_create("websocket:room-scope")
    session.metadata["conversation"] = ConversationMetadata.room(
        ["mona", "com.mona.a-share-analyst"], title="scope test"
    ).to_session_metadata()
    loop.sessions.save(session)
    loop.context.build_messages = MagicMock(  # type: ignore[method-assign]
    ...
    first = InboundMessage(
        ...
        sender_id="u1",
        chat_id="room-scope",
        content="@Mona @A 请分别介绍自己",
        metadata={
            DIRECT_TARGET_AGENT_IDS_META: ["mona", "com.mona.a-share-analyst"],
            PARTNER_JOBS_DISPATCHED_META: ["com.mona.a-share-analyst"],
        },
    )
    await loop._process_message(first)

    assert "Internal direct-mention boundary" in seen[0][0]["content"]
    assert "不要介绍、代答或模拟其他被直接 @ 的成员" in seen[0][0]["content"]
    assert seen[0][1]["content"] == first.content

    persisted = loop.sessions.get_or_create("websocket:room-scope")
    assert persisted.messages[0]["role"] == "user"
    assert persisted.messages[0]["content"] == first.content
    ...
    assert len(seen) == 2
    assert "Internal direct-mention boundary" not in seen[1][0]["content"]
...
async def test_stop_preserves_runtime_checkpoint_for_next_turn(tmp_path: Path) -> None:
    from mona.command.builtin import cmd_stop
    from mona.command.router import CommandContext

    loop = _make_full_loop(tmp_path)
    loop.tools = ToolRegistry()
    loop._refresh_provider_snapshot = MagicMock()  # type: ignore[method-assign]
    loop._refresh_mona_user_config = MagicMock()  # type: ignore[method-assign]
    loop._refresh_media_generation_tools = MagicMock()  # type: ignore[method-assign]
    loop._refresh_subscription_access = AsyncMock()  # type: ignore[method-assign]
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    def build_test_messages(**kwargs):
    ...
    loop.context.build_messages = MagicMock(  # type: ignore[method-assign]
        side_effect=build_test_messages
    )
...
class _RecoveryOfficeClient:
    def __init__(self) -> None:
        self.apply_commands: list[OfficeApplyCommand] = []
    ...
    async def apply(self, command: OfficeApplyCommand, *, owner_session_key: str):
        ...
        if len(self.apply_commands) == 1:
            return OfficeCommandFailure(
                ok=False,
                session_id=command.session_id,
                operation_id=command.operation_id,
                current_version=DocumentVersion(editor_epoch="epoch_recovery", model_revision=1),
                changed_targets=[],
                error={
                    "code": "INVALID_OPERATION",
                    "message": "the first apply intentionally failed",
                    "retryable": False,
                },
            )
        return OfficeCommandSuccess(
            ...
            session_id=command.session_id,
            operation_id=command.operation_id,
            version=DocumentVersion(editor_epoch="epoch_recovery", model_revision=2),
            changed_targets=["Sheet1!A1"],
            summary="the second apply succeeded",
        )
    ...
    async def inspect(self, request: OfficeInspectRequest, *, owner_session_key: str):
        ...
        return OfficeInspectSuccess(
            ...
            result=VisualResult(
                ...
                width=2,
                height=1,
                target=f"page-{page_index}",
            ),
        )


@pytest.mark.asyncio
async def test_real_runner_checkpoint_recovery_keeps_early_office_results_and_visual_placeholder(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_office = _RecoveryOfficeClient()
    monkeypatch.setattr(
        "mona.agent.tools.office.OfficeServiceClient.from_port",
        lambda _port: fake_office,
    )
    loop = _make_full_loop(tmp_path)
    office = OfficeTool(workspace=tmp_path)
    tools = ToolRegistry()
    tools.register(office)
    loop.tools = tools

    session_key = "websocket:office-recovery"
    session = loop.sessions.get_or_create(session_key)
    session.add_message("user", "recover this Office turn")
    loop.sessions.save(session)
    loop._set_tool_context(
        "websocket",
        "office-recovery",
    ...
    expected_version = {"editorEpoch": "epoch_recovery", "modelRevision": 1}
    apply_operation = {
        "op": "set_cell",
        "payload": {"sheet": "Sheet1", "cell": "A1", "value": "value"},
    }
    responses = [
        LLMResponse(
            content="apply once",
    ...
    responses.extend(
        LLMResponse(
            content=f"inspect {page_index}",
            tool_calls=[
                ToolCallRequest(
                    id=f"inspect-{page_index}",
                    name="office",
                    arguments={
                        "action": "inspect",
                        "session_id": "office-recovery",
                        "query": {"mode": "visual", "pageIndex": page_index},
                    },
                )
            ],
        )
    ...
    responses.append(LLMResponse(content="completed after recovery"))
    loop.provider.chat_with_retry = AsyncMock(side_effect=responses)
    progress_events: list[dict[str, object]] = []

    async def on_progress(_content: str, **kwargs: object) -> None:
        events = kwargs.get("tool_events")
        if isinstance(events, list):
            progress_events.extend(event for event in events if isinstance(event, dict))

    result = await loop._run_agent_loop(
        [
            {"role": "system", "content": "Use the Office tool."},
            {"role": "user", "content": "recover this Office turn"},
        ],
        session=session,
        channel="websocket",
        chat_id="office-recovery",
        session_key=session_key,
        on_progress=on_progress,
    )

    assert result[0] == "completed after recovery"
    assert loop.provider.chat_with_retry.await_count == 20
    assert len(fake_office.apply_commands) == 2
    assert len(fake_office.inspect_requests) == 17
    finish_events = [event for event in progress_events if event.get("phase") in {"end", "error"}]
    assert any(event.get("call_id") == "apply-failure" and event.get("phase") == "error" for event in finish_events)
    assert any(event.get("call_id") == "apply-success" and event.get("phase") == "end" for event in finish_events)

    loop.sessions.invalidate(session_key)
    reloaded = loop.sessions.get_or_create(session_key)
    checkpoint = reloaded.metadata.get(AgentLoop._RUNTIME_CHECKPOINT_KEY)
    assert isinstance(checkpoint, dict)
    assert checkpoint["phase"] == "final_response"
    assert checkpoint["pending_tool_calls"] == []
    assert len(checkpoint["completed_messages"]) >= 38
    assert loop._restore_runtime_checkpoint(reloaded) is True
    ...
    failure = json.loads(tool_messages["apply-failure"]["content"])
    success = json.loads(tool_messages["apply-success"]["content"])
    assert failure["ok"] is False
    assert failure["error"]["code"] == "INVALID_OPERATION"
    ...
    assert len(visual_messages) == 17
    final_visual = visual_messages[-1]["content"]
    assert [block["type"] for block in final_visual] == ["text", "text"]
    visual_payload = json.loads(final_visual[0]["text"])
    assert visual_payload["result"]["mode"] == "visual"
    assert "dataUrl" not in visual_payload["result"]
    assert final_visual[1] == {"type": "text", "text": "[image]"}
