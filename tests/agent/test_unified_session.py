...
from mona.command.builtin import cmd_new, register_builtin_commands
from mona.command.router import CommandContext, CommandRouter
from mona.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("mona.agent.loop.SessionManager"), \
         patch("mona.agent.loop.SubagentManager") as MockSubMgr, \
         patch("mona.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates mona onboard)."""
        from mona.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from mona.agent.memory import Consolidator, MemoryStore
