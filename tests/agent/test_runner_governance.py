...
def _make_loop(tmp_path):
    ...
    with patch("mona.agent.loop.ContextBuilder"), \
         patch("mona.agent.loop.SessionManager"), \
         patch("mona.agent.loop.SubagentManager") as MockSubMgr:
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("mona.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "mona.agent.runner.estimate_message_tokens",
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from mona.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from mona.agent.loop import AgentLoop
    from mona.agent.runner import _BACKFILL_CONTENT
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from mona.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from mona.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    monkeypatch.setattr("mona.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
