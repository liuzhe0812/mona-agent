...
from mona.providers.openai_codex_provider import _strip_model_prefix
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'mona agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"mona gateway --config {resolved_config}" in compact_output
...
def test_provider_logout_paths_resolve_to_expected_files():
    ...
    from mona.providers.github_copilot_provider import get_storage
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("mona.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from mona.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("mona.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("mona.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
        provider = make_provider(config)
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("mona.config.loader.load_config", return_value=config) as mock_load_config, \
         ...
         patch("mona.cron.service.CronService"), \
         patch("mona.cli.commands.AgentLoop.from_config") as mock_from_config:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "mona.config.loader.set_config_path",
    ...
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mona.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("mona.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mona.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "mona.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr(
        "mona.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )

    if message_bus is not None:
        monkeypatch.setattr("mona.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("mona.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("mona.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("mona.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mona.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("mona.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("mona.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("mona.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mona.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "mona.utils.evaluator.evaluate_response",
...
def test_gateway_cron_job_suppresses_intermediate_progress(
    ...
    monkeypatch.setattr("mona.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("mona.session.manager.SessionManager", lambda _workspace: object())
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from mona.cli.commands import _migrate_cron_store
    ...
    with patch("mona.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("mona.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mona.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("mona.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("mona.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
