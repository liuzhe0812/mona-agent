...
from mona.config.schema import ChannelsConfig
from mona.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from mona.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from mona.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from mona.channels.registry import discover_plugins
...
def test_discover_enabled_imports_only_enabled_builtins():
    from mona.channels.registry import discover_enabled
    ...
    with (
        patch("mona.channels.registry.load_channel_class", side_effect=_load_channel),
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "mona.channels.registry.discover_enabled",
...
async def test_base_channel_passes_api_base_to_openai_transcription_provider():
    ...
    from mona.providers import transcription as transcription_mod
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("mona.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("mona.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "mona.channels.registry.discover_all",
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("mona.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("mona.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("mona.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("mona.channels.manager.consume_restart_notice_from_env", return_value=notice):
