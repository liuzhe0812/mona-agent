...
try:
    from mona.channels import dingtalk
...
import mona.channels.dingtalk as dingtalk_module
from mona.bus.queue import MessageBus
from mona.channels.dingtalk import DingTalkChannel, DingTalkConfig, monaDingTalkHandler
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = monaDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/mona_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/mona_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mona.config.paths.get_media_dir",
