...
from mona.config.schema import WebFetchConfig
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("mona.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("mona.security.network.socket.getaddrinfo", _fake_resolve_public), \
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    ...
    monkeypatch.setattr("mona.agent.tools.web.httpx.AsyncClient", FakeClient)
