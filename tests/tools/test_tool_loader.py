...
from mona.agent.tools.loader import ToolLoader, _SKIP_MODULES
...
def test_fs_tool_create_builds_from_context():
    from mona.agent.tools.filesystem import ReadFileTool
...
def test_spawn_tool_create():
    from mona.agent.tools.spawn import SpawnTool
    mock_mgr = MagicMock()
    mock_config = MagicMock()
...
def test_exec_tool_config_cls():
    from mona.agent.tools.shell import ExecTool, ExecToolConfig
...
def test_web_tools_config_cls():
    from mona.agent.tools.web import WebSearchTool, WebFetchTool, WebToolsConfig
...
def test_my_tool_config_cls():
    from mona.agent.tools.self import MyTool, MyToolConfig
...
def test_mcp_wrappers_not_discoverable():
    from mona.agent.tools.mcp import MCPToolWrapper, MCPResourceWrapper, MCPPromptWrapper
