...
from mona.providers.image_generation import (
    ...
    OpenAICompatImageGenerationClient,
...
def test_image_generation_provider_configs_skip_cindy_mapping() -> None:
    config = Config(
    ...
    assert result["openrouter"].api_key == "sk-or-test"
...
async def test_aihubmix_image_generation_payload_and_response() -> None:
    ...
    assert call["headers"]["APP-Code"] == "mona"
    assert call["json"] == {
        "input": {
            "prompt": "draw a logo",
            "n": 1,
...
@pytest.mark.asyncio
async def test_aihubmix_default_1k_uses_existing_orientation_mapping() -> None:
    ...
    assert fake.calls[0]["json"]["input"]["size"] == "1024x1024"
...
@pytest.mark.asyncio
async def test_gemini_imagen_rejects_unsupported_aspect_ratio() -> None:
    fake = FakeClient(
        FakeResponse({"predictions": [{"bytesBase64Encoded": RAW_B64, "mimeType": "image/png"}]})
    )
    client = GeminiImageGenerationClient(api_key="AIza-test", client=fake)  # type: ignore[arg-type]

    with pytest.raises(ImageGenerationError, match="does not support aspect ratio"):
        await client.generate(
            prompt="a sunset",
            model="imagen-4.0-generate-001",
            aspect_ratio="2:3",
...
@pytest.mark.asyncio
async def test_minimax_omits_aspect_ratio_when_unspecified() -> None:
    fake = FakeClient(FakeResponse({"data": {"image_base64": [RAW_B64]}}))
    client = MiniMaxImageGenerationClient(
        api_key="sk-mm-test",
        client=fake,  # type: ignore[arg-type]
    ...
    await client.generate(prompt="draw", model="image-01")

    assert "aspect_ratio" not in fake.calls[0]["json"]


@pytest.mark.asyncio
async def test_minimax_rejects_explicit_image_size() -> None:
    fake = FakeClient(FakeResponse({"data": {"image_base64": [RAW_B64]}}))
    client = MiniMaxImageGenerationClient(
        api_key="sk-mm-test",
        client=fake,  # type: ignore[arg-type]
    ...
    with pytest.raises(ImageGenerationError, match="does not support this image_size"):
        await client.generate(prompt="draw", model="image-01", image_size="1024x1536")
    assert fake.calls == []
...
@pytest.mark.asyncio
async def test_stepfun_omits_size_when_no_aspect_ratio() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = StepFunImageGenerationClient(
        api_key="sk-sf-test",
        api_base="https://api.stepfun.com/v1",
...
@pytest.mark.asyncio
async def test_stepfun_default_1k_uses_existing_orientation_mapping() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = StepFunImageGenerationClient(
        api_key="sk-sf-test",
        client=fake,  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_openai_default_1k_tier_uses_known_pixel_mapping() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        client=fake,  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_openai_unsupported_tier_exposes_known_pixel_sizes() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        client=fake,  # type: ignore[arg-type]
    ...
    assert error.supported_sizes == ["1024x1024", "1024x1792", "1792x1024"]
...
@pytest.mark.asyncio
async def test_openai_omits_size_when_no_aspect_ratio() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        client=fake,  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_openai_rejects_explicit_size_unsupported_by_model_family() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        client=fake,  # type: ignore[arg-type]
    ...
    with pytest.raises(ImageGenerationError, match="not supported"):
        await client.generate(
            prompt="draw",
            model="dall-e-3",
            aspect_ratio="16:9",
...
@pytest.mark.asyncio
async def test_openai_extra_body_cannot_override_request_fields() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        extra_body={
    ...
    await client.generate(
        prompt="draw the requested image",
        model="dall-e-3",
    ...
    body = fake.calls[0]["json"]
    assert body["model"] == "dall-e-3"
    assert body["prompt"] == "draw the requested image"
    assert body["size"] == "1792x1024"
    assert body["quality"] == "high"


@pytest.mark.asyncio
async def test_openai_extra_body_size_is_preserved_when_request_omits_size() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAIImageGenerationClient(
        api_key="sk-openai-test",
        extra_body={"size": "1792x1024"},
        client=fake,  # type: ignore[arg-type]
    )

    await client.generate(prompt="draw", model="dall-e-3")

    assert fake.calls[0]["json"]["size"] == "1792x1024"
...
@pytest.mark.asyncio
async def test_openai_compat_omits_output_size_when_unspecified() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
...
@pytest.mark.asyncio
async def test_openai_compat_preserves_preconfigured_output_when_unspecified() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        extra_body={"size": "service-default", "quality": "high"},
        client=fake,  # type: ignore[arg-type]
    )
    ...
    body = fake.calls[0]["json"]
    assert body["size"] == "service-default"
    assert body["quality"] == "high"


@pytest.mark.asyncio
async def test_openai_compat_self_hosted_size_is_not_restricted_by_model_name() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        client=fake,  # type: ignore[arg-type]
    ...
    assert fake.calls[0]["json"]["size"] == "768x1024"


@pytest.mark.asyncio
async def test_openai_compat_size_and_ratio_are_forwarded_without_remapping() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        client=fake,  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_openai_compat_size_mode_forwards_advertised_tier() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        client=fake,  # type: ignore[arg-type]
    ...
    await client.generate(prompt="draw", model="z-image", image_size="2K")

    assert fake.calls[0]["json"]["size"] == "2K"


@pytest.mark.asyncio
async def test_openai_compat_aspect_ratio_is_forwarded_for_self_hosted_service() -> None:
    fake = FakeClient(FakeResponse({"data": [{"b64_json": RAW_B64}]}))
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        client=fake,  # type: ignore[arg-type]
    ...
    await client.generate(prompt="draw", model="z-image", aspect_ratio="9:16")
...
async def test_openai_compat_selected_size_removes_preconfigured_size_aliases() -> None:
    ...
    await client.generate(prompt="draw", model="z-image", image_size="768x1024")

    body = fake.calls[0]["json"]
    assert body["size"] == "768x1024"
    for key in ("width", "height", "resolution", "aspect_ratio", "image_config"):
        assert key not in body


@pytest.mark.asyncio
async def test_openai_compat_structured_size_error_is_recoverable() -> None:
    fake = FakeClient(
        FakeResponse(
            {
                "error": {
                    ...
                    "allowed_sizes": ["1024x1024", "1152x2048"],
                }
            },
            status_code=422,
        )
    ...
    with pytest.raises(ImageGenerationError) as caught:
        await client.generate(prompt="draw", model="gpt-image-2", image_size="1600x900")
...
async def test_output_parameter_rejection_preserves_message_without_inventing_options(status) -> None:
    fake = FakeClient(FakeResponse({"error": {
        "param": "size", "type": "invalid_request_error", "message": "This deployment accepts pixel dimensions only",
    }}, status_code=status))
    client = OpenAICompatImageGenerationClient(api_key=None, client=fake)  # type: ignore[arg-type]
    with pytest.raises(ImageGenerationError) as caught:
        await client.generate(prompt="draw", model="local", image_size="2K")
    assert caught.value.code == "UNSUPPORTED_IMAGE_SIZE"
    assert caught.value.retry_safe is True
    assert caught.value.supported_sizes == []
    assert "pixel dimensions only" in str(caught.value)
    assert len(fake.calls) == 1


@pytest.mark.asyncio
async def test_openai_compat_auth_error_with_sizes_is_not_size_error() -> None:
    fake = FakeClient(
        FakeResponse(
            {
                "error": {
    ...
    error = caught.value
    assert error.code == "IMAGE_AUTHENTICATION_FAILED"
    assert error.supported_sizes == []
    assert error.retry_safe is False


class RaisingPostClient:
    async def post(self, url: str, **kwargs: Any) -> Any:
        raise httpx.ReadTimeout("connection timed out")


@pytest.mark.asyncio
async def test_openai_compat_transport_error_is_uncertain_and_not_retryable() -> None:
    client = OpenAICompatImageGenerationClient(
        api_key=None,
        client=RaisingPostClient(),  # type: ignore[arg-type]
    )
    ...
    error = caught.value
    assert error.code == "IMAGE_GENERATION_UNCERTAIN"
    assert error.retry_safe is False
    assert "cannot confirm whether the request was submitted" in str(error)


@pytest.mark.asyncio
async def test_openai_compat_task_timeout_keeps_task_id() -> None:
    client = OpenAICompatImageGenerationClient(api_key=None, timeout=0)

    with pytest.raises(ImageGenerationError) as caught:
        await client._poll_task_images(FakeClient(FakeResponse({})), "task-42", {})

    error = caught.value
    assert error.code == "IMAGE_GENERATION_UNCERTAIN"
    assert error.task_id == "task-42"
    assert "task_id=task-42" in str(error)
    assert error.retry_safe is False
