...
async def test_codex_prompt_cache_key_uses_stable_conversation_prefix(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mona.providers.openai_codex_provider.get_codex_token",
    ...
    monkeypatch.setattr("mona.providers.openai_codex_provider._request_codex", fake_request)

    provider = OpenAICodexProvider()
    await provider.chat(
        [
            {"role": "system", "content": "You are mona."},
