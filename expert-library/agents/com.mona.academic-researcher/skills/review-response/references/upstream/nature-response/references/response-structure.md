...
When modifying manuscript text, work on a copy of the original manuscript and mark changed passages in red. For LaTeX manuscripts, wrap changed text in `\revised{...}` from `upstream/nature-response/templates/revised-manuscript-redline.tex`. For non-LaTeX outputs, use a clear red-text convention appropriate to the target format and keep a clean version separate from the marked copy.
...
When the user asks for LaTeX, use the templates in `upstream/nature-response/templates/`:

- `upstream/nature-response/templates/cover-letter.tex` for revision cover letters.
- `upstream/nature-response/templates/response-to-reviewers.tex` for point-by-point responses.
- `upstream/nature-response/templates/revised-manuscript-redline.tex` for a red-marked manuscript-copy skeleton.

Fill one copy of `upstream/nature-response/templates/response-to-reviewers.tex` per reviewer. Never place multiple mutually
