...
- A **static layer** under `upstream/nature-response/static/` that holds versioned, reusable content fragments (the default stance and red lines, and the response workflow with output format).
- A **dynamic layer** (this file plus `upstream/nature-response/manifest.yaml`) that loads the core every time and reaches for the deeper response references or templates only when a step needs them.
...
Read [manifest.yaml](upstream/nature-response/manifest.yaml). Then read every file listed under `always_load`:

- `upstream/nature-response/static/core/stance.md` — the editor-facing purpose, the default stance, the red lines, and the source hierarchy that apply to every response job.
- `upstream/nature-response/static/core/workflow.md` — accepted inputs, the revision correspondence workflow, and the output package format.
...
Use `upstream/nature-response/references/intake-and-routing.md` to fix the task mode, minimum inputs, and readiness state before drafting. Route appeal-like cases separately; do not draft an appeal as the default path.
...
`upstream/nature-shared/core/main-text-discipline.md`. Answer the reviewer completely
...
The files under `upstream/nature-response/references/` and `upstream/nature-response/templates/` are deep resources, not defaults. Open them on demand per the `references.on_demand` table in the manifest — for example `upstream/nature-response/references/comment-taxonomy.md` to classify comments, `upstream/nature-response/references/action-mapping.md` for tracker fields, `upstream/nature-response/references/tone-and-stance.md` for disagreement wording, `upstream/nature-response/references/difficult-cases.md` for impossible experiments / conflicting reviewers / appeal-like cases, `upstream/nature-response/references/chinese-author-alignment.md` for Chinese author notes, `upstream/nature-response/references/latex-templates.md` for `.tex` cover/response/redline outputs, `upstream/nature-shared/core/main-text-discipline.md` for reviewer-driven manuscript additions and evidence relocation, `upstream/nature-response/references/package-consistency-audit.md` whenever the manuscript is edited alongside the letter or the package is about to be compiled and delivered, and `upstream/nature-response/references/qa-checklist.md` before finalizing.
