...
5. Inside each isolated review, independently assess readiness and the source-grounded axes, then build that reviewer's own concern ledger using `upstream/nature-reviewer/references/technical-concern-taxonomy.md`. If relevant, load only the applicable section of `upstream/nature-reviewer/references/domain-specific-review-gates.md` inside that same isolated context.
...
| File | Open when |
|---|---|
| [references/source-basis.md](upstream/nature-reviewer/references/source-basis.md) | You need source provenance, local rule summaries, or source-vs-implementation boundaries |
| [references/reviewer-workflow.md](upstream/nature-reviewer/references/reviewer-workflow.md) | You need the invocation order, fact-base extraction flow, or synthesis rules |
| [references/review-axes.md](upstream/nature-reviewer/references/review-axes.md) | You need the evaluation axes or reviewer weighting logic |
| [references/technical-concern-taxonomy.md](upstream/nature-reviewer/references/technical-concern-taxonomy.md) | You need the internal 12-axis coverage check, concern ledger, or claim/evidence-pointer rules |
| [references/domain-specific-review-gates.md](upstream/nature-reviewer/references/domain-specific-review-gates.md) | The manuscript has clear chemistry, engineering, materials, atmospheric, climate-ecology, hydrology, or remote-sensing evidence chains |
| [references/report-structure.md](upstream/nature-reviewer/references/report-structure.md) | You need the default output contract or section anatomy |
| [references/role-boundaries.md](upstream/nature-reviewer/references/role-boundaries.md) | You need constraints on reviewer differences and editor-versus-reviewer boundaries |
| [references/qa-checklist.md](upstream/nature-reviewer/references/qa-checklist.md) | You are finalizing an output and need groundedness / non-invention checks |
...
1. `upstream/nature-reviewer/references/editorial criteria and processes.md`
2. manuscript facts supplied by the user
3. conservative local implementation rules documented in `upstream/nature-reviewer/references/source-basis.md`
4. domain-specific supporting gates in `upstream/nature-reviewer/references/domain-specific-review-gates.md`
