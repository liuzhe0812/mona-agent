...
- A **static layer** under `upstream/nature-reader/static/` that holds versioned, reusable content fragments (core principles, the reading workflow, the output contract, and per-source-format extraction guidance).
- A **dynamic layer** (this file plus `upstream/nature-reader/manifest.yaml`) that detects the request's source format and loads only the fragments needed for the current job.
...
Read [manifest.yaml](upstream/nature-reader/manifest.yaml). It declares the `source_format` axis, the allowed values, and the file paths each value maps to.
...
Read the file mapped for the detected `source_format`. Do **not** read every fragment in `upstream/nature-reader/static/`. Load only what step 2 selected.
...
Build the Terminology Ledger as you translate (`upstream/nature-shared/core/terminology-ledger.md`); it becomes the `paper.md` recurring-term table and the `source_map.json` glossary.
...
The files under `upstream/nature-reader/references/` are deep references, not defaults. Open them on demand per the `references.on_demand` table in the manifest:

- detailed figure/table cropping and placement → `upstream/nature-reader/references/figure-extraction.md`.
- exact field schema for `paper.md` / `source_map.json` → `upstream/nature-reader/references/output-spec.md`.
- equations, mathematical expressions, chemical formulae, or image-only formulae → `upstream/nature-reader/references/equation-handling.md`.
- answering follow-up questions with source citations → `upstream/nature-reader/references/grounding-rules.md`.
