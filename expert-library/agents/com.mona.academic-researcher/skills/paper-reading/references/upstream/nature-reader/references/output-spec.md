...
Use `$...$` for inline math and `$$...$$` for display math. For a low-confidence or image-only equation, show the original crop from `assets/equations/` and label any accompanying LaTeX as a low-confidence transcription. See `upstream/nature-reader/references/equation-handling.md` for the exact block shape.
