...
- Use native math objects or embedded LaTeX when available. If visual verification still leaves ambiguity, crop the original equation and use the low-confidence fallback in `upstream/nature-reader/references/equation-handling.md`.
- Figures and tables are images embedded in the page — crop them per `upstream/nature-reader/references/figure-extraction.md`; do not paste the page text of a table where the table image belongs.
