...
- On first contact with the draft, build a Terminology Ledger and keep terms, abbreviations, units, and notation consistent across every section. Do not introduce synonyms to vary the prose. See `upstream/nature-shared/core/terminology-ledger.md`.
...
See `upstream/nature-shared/core/reader-workflow.md` (loaded via manifest `always_load`) for the 5-step reader question sequence. Polishing should help the paper answer those questions in order.
