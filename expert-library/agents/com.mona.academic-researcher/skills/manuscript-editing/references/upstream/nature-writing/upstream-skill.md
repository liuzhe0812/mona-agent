...
- A **static layer** under `upstream/nature-writing/static/` that holds versioned, reusable content fragments (core stance + workflow, paper-type playbooks, per-section drafting guidance, initial-submission guidance, language-specific rules, per-journal style).
- A **dynamic layer** (this file plus `upstream/nature-writing/manifest.yaml`) that detects the request's axes and loads only the fragments needed for the current job.
...
Read [manifest.yaml](upstream/nature-writing/manifest.yaml). It declares the axes (`task`, `paper_type`, `section`, `language`, `journal`), the allowed values, and the file paths each value maps to.
...
Do **not** read every fragment in `upstream/nature-writing/static/`. Load only what step 2 selected.
...
text, also load `upstream/nature-shared/core/main-text-discipline.md` before building
...
- Results or Discussion →
  `upstream/nature-shared/core/nature-results-discussion.md`
- Introduction or whole-manuscript narrative →
  `upstream/nature-shared/core/nature-introduction.md`
- Abstract → `upstream/nature-shared/core/nature-abstract.md`
...
`upstream/nature-shared/core/discussion-argument-language.md`. Use it to select the
...
For `task=submission-package`, follow `upstream/nature-writing/static/fragments/task/submission-package.md` and `upstream/nature-writing/references/submission-package.md` instead. Build the deliverable matrix and readiness audit; do not force manuscript paragraph architecture onto administrative submission materials.
...
The files under `upstream/nature-writing/references/` are deep references and the example library, not defaults. Open them on demand per the `references.on_demand` table in the manifest. Typical triggers:

- The user asks for a concrete example or template → `upstream/nature-writing/references/examples/index.md`.
- A section's draft has structural problems that the section fragment alone does not explain → the matching `upstream/nature-writing/references/<section>.md`.
- The user needs a broad-audience `Nature` abstract opening or asks about a `summary paragraph` → `upstream/nature-writing/references/nature-summary-paragraph.md`.
- The user asks "does this paragraph flow?" → `upstream/nature-writing/references/paragraph-flow.md`.
- The user asks for a self-review or rejection-risk audit → `upstream/nature-writing/references/paper-review.md`.
- The user asks what belongs in the main text, captions, or SI; wants a shorter
  ...
  `upstream/nature-shared/core/main-text-discipline.md`.
- The user requests a complete first-submission package, templates, or a submission-readiness audit → `upstream/nature-writing/references/submission-package.md`.
- The target is the flagship journal Nature and exact submission or formatting
  requirements matter → `upstream/nature-shared/journal-formats/nature.md`.
- The target is Nature Machine Intelligence and exact content-type, submission,
  data/code or production requirements matter →
  `upstream/nature-shared/journal-formats/nature-machine-intelligence.md`.
...
- Any target needs a Discussion function chain, evidence-calibrated modal
  ...
  `upstream/nature-shared/core/discussion-argument-language.md`.
- Any Nature / Nature Portfolio target needs an Introduction funnel, exact gap,
  ...
  Results → `upstream/nature-shared/core/nature-introduction.md`.
- Any Nature / Nature Portfolio target needs abstract evidence-chain,
  ...
  `upstream/nature-shared/core/nature-abstract.md`.
- The work involves regulated or specialist research compliance →
  `upstream/nature-shared/core/research-compliance.md`.
