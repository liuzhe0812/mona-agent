...
报告措辞时，读取 `upstream/nature-ref-verifier/references/common-patterns.md`。其他任务不要预加载该文件；
`upstream/nature-ref-verifier/manifest.yaml` 记录了同一按需路由，新增资源时必须同时在本文件中写明加载条件。
