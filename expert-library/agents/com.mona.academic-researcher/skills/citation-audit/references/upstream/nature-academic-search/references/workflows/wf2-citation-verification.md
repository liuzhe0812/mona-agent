...
**Uses:**
- [Citation Parser](upstream/nature-academic-search/references/citation-parser.md) — extraction strategies per source format.
- [Dedup Engine](upstream/nature-academic-search/references/dedup-engine.md) — collapse duplicate candidate matches before classification.

## Procedure

1. **Extract citations** from document using [Citation Parser](upstream/nature-academic-search/references/citation-parser.md).
   Prefer T1 sources for primary verification (CrossRef DOI lookup → PubMed PMID confirmation). Use T2 (Semantic Scholar) for cross-checking ambiguous or missing results. See [Source Tiers](upstream/nature-academic-search/references/source-tiers.md) for full routing.
...
4. **Classify** into: `verified` | `mismatch` | `not_found` | `suspicious` | `manual_needed`.
   See [Citation Parser: Classification Labels](upstream/nature-academic-search/references/citation-parser.md#classification-labels) for criteria.
5. **Generate report:**
