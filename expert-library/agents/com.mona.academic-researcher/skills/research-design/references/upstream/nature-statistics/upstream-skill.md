...
5. **Check common failure modes.** Use `upstream/nature-statistics/references/common-failure-modes.md` when the text involves nested data, many comparisons, cell-level measurements, interaction claims, correlations, regression, outliers, small samples, or significance-only reasoning.
6. **Check reporting completeness.** Use `upstream/nature-statistics/references/statistical-reporting.md` to verify that Methods and Results give enough information for readers and reviewers to understand the analysis.
   If the target is the flagship journal Nature, also use
   `upstream/nature-statistics/references/nature-article-requirements.md` for its exact tail, `n`, repeat,
   P-value, test-statistic and degrees-of-freedom requirements.
   If the target is Nature Machine Intelligence, also use
   `upstream/nature-shared/journal-formats/nature-machine-intelligence.md` for its
   legend-statistics, source-data, reporting-summary and stage-specific checks.
7. **Align figure statistics.** Use `upstream/nature-statistics/references/figure-statistics.md` when figure legends, panel labels, stars, error bars, box plots, violin plots, source data, or supplementary figure notes are involved.
...
9. **Run final QA.** Use `upstream/nature-statistics/references/reviewer-checklist.md` before final delivery for severity labels, unresolved author questions, and reviewer-facing risk.
...
| File | Open when |
|---|---|
| [references/source-basis.md](upstream/nature-statistics/references/source-basis.md) | You need the source hierarchy or want to justify why the skill emphasizes transparency, reproducibility, and design reporting |
| [references/nature-article-requirements.md](upstream/nature-statistics/references/nature-article-requirements.md) | The target is the flagship journal Nature or the user requests its exact statistical submission checklist |
| [upstream/nature-shared/journal-formats/nature-machine-intelligence.md](upstream/nature-shared/journal-formats/nature-machine-intelligence.md) | The target is Nature Machine Intelligence or NMI-specific legend, source-data, reporting or stage requirements affect the audit |
| [references/statistical-reporting.md](upstream/nature-statistics/references/statistical-reporting.md) | You are drafting or auditing Statistical analysis, Methods, Results, or Supplementary Methods text |
| [references/common-failure-modes.md](upstream/nature-statistics/references/common-failure-modes.md) | You see nested measurements, many comparisons, interaction claims, correlation/regression, outliers, tiny samples, or overstrong p-value language |
| [references/figure-statistics.md](upstream/nature-statistics/references/figure-statistics.md) | You are checking figure legends, panel statistics, error bars, stars, box/violin plots, source-data notes, or graphical reporting |
| [references/reviewer-checklist.md](upstream/nature-statistics/references/reviewer-checklist.md) | You are finalizing an audit or preparing a reviewer-facing risk summary |
| [upstream/nature-shared/core/consistency-sweep.md](upstream/nature-shared/core/consistency-sweep.md) | The same statistic appears in more than one place, or interval terminology is in question: one metric at two precisions across table and text, SD/Std abbreviation drift, `confidence interval` used where `prediction interval` is meant, or overlapping error bars described as outperformance |
...
3. Nature Methods / Nature Portfolio statistics guidance summarized in `upstream/nature-statistics/references/source-basis.md`.
