...
If the user explicitly requests language-only review and the diagnosis finds no content-level risk, skip professor dispatch. Load `upstream/nature-proposal-writer/references/research-anti-slop.md`, run the language scan, and produce a focused revision brief. Do not manufacture a scientific review stage when it cannot change the result.
