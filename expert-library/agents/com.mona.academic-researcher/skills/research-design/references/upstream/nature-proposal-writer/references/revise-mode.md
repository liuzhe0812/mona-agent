...
- For `full_proposal`, load `upstream/nature-proposal-writer/references/partial-proposal-scope.md` if the document is intentionally partial-stage (e.g., Part 1 only). Don't flag missing later-stage content as a defect.
...
**Process**: if the structural diagnosis reveals non-trivial content risks (unsupported claims in key positions, scope violations, argument gaps), dispatch professor with 1-2 specialists. See `upstream/nature-proposal-writer/references/professor-dispatch.md`.
...
**Process**: scan for patterns listed in `upstream/nature-proposal-writer/references/research-anti-slop.md`. Mark each occurrence with:
