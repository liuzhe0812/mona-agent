...
export function parseArgs(argv) {
  ...
  if (!path.isAbsolute(a.out)) throw new Error("--out must be an absolute path");
  a.out = path.resolve(a.out);
  const workspace = process.env.MONA_ACTIVE_WORKSPACE;
  if (workspace) {
    const workspaceRoot = path.resolve(workspace);
    const relative = path.relative(workspaceRoot, a.out);
    if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
      throw new Error("--out must stay inside the active Mona workspace");
    }
  }
  parseSiChoice(a);
