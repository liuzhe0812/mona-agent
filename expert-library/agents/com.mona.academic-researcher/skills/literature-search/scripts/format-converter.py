...
refs.txt format:
  ...
  # Lines starting with # are comments
    ...
    parser.add_argument(
        "--output",
        help="Required absolute output directory inside the active Mona workspace",
    )
    ...
    if not args.output:
        parser.error("--output is required for conversion or download operations")
    if not os.path.isabs(args.output):
        parser.error("--output must be an absolute path")

    fmt = args.format
    output_dir = os.path.realpath(args.output)
    workspace = os.environ.get("MONA_ACTIVE_WORKSPACE")
    if workspace:
        workspace = os.path.realpath(workspace)
        try:
            inside_workspace = os.path.commonpath([workspace, output_dir]) == workspace
        except ValueError:
            inside_workspace = False
        if not inside_workspace:
            parser.error("--output must stay inside the active Mona workspace")
