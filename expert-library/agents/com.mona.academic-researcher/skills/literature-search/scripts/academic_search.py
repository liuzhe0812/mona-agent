...
Source: OpenAlex (https://openalex.org). Small anonymous requests may work, but
normal product use should provide the free `OPENALEX_API_KEY`. OpenAlex indexes
...
def _with_api_key(params: dict) -> dict:
    """Attach the locally configured OpenAlex key without exposing it."""
    key = os.environ.get("OPENALEX_API_KEY", "").strip()
    return {**params, **({"api_key": key} if key else {})}
...
def fetch_author_candidates(name: str, per_page: int = 50) -> list[dict]:
    params = _with_api_key({"search": name, "per_page": per_page, "mailto": MAILTO})
...
def resolve_by_orcid(orcid: str) -> dict | None:
    ...
    if set(oid) <= {"0", "-"}:
        return None
    params = _with_api_key({"filter": f"orcid:{oid}", "mailto": MAILTO})
    url = f"{AUTHORS_API}?{urllib.parse.urlencode(params)}"
    req = urllib.request.Request(url, headers={"Accept": "application/json"})
    with urllib.request.urlopen(req, timeout=30) as resp:
        data = json.loads(resp.read().decode())
