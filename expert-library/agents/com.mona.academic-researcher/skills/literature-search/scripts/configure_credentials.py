...
ENV_FIELDS = {
    "elsevier": {"api_key": "ELSEVIER_API_KEY", "insttoken": "ELSEVIER_INSTTOKEN", "authtoken": "ELSEVIER_AUTHTOKEN"},
    "springer_nature": {"api_key": "SPRINGER_NATURE_API_KEY"},
    "ieee": {"api_key": "IEEE_API_KEY", "fulltext_endpoint": "IEEE_FULLTEXT_ENDPOINT"},
}


def _environment_credentials(provider: str) -> dict:
    values = {
        key: os.environ.get(env_name, "").strip()
        for key, env_name in ENV_FIELDS[provider].items()
        if os.environ.get(env_name, "").strip()
    }
    return values if values.get("api_key") else {}
...
def cmd_set(args: argparse.Namespace) -> int:
    if sys.platform == "win32":
        print(json.dumps({"ok": False, "status": "secure_store_required", "message": "Use Mona credential settings or provider environment variables; plaintext credential files are disabled on Windows."}, ensure_ascii=False))
        return 2
    if args.stdin:
...
def cmd_show(_: argparse.Namespace) -> int:
    ...
    for provider in PROVIDERS:
        values = _environment_credentials(provider)
        if values:
...
def cmd_delete(args: argparse.Namespace) -> int:
    if sys.platform == "win32":
        print(json.dumps({"ok": False, "status": "secure_store_required", "message": "Remove the credential in Mona settings or the provider environment variable."}, ensure_ascii=False))
        return 2
    credentials = _load(CREDENTIALS_FILE)
...
def cmd_validate(args: argparse.Namespace) -> int:
    values = _environment_credentials(args.provider) or _load(CREDENTIALS_FILE).get(args.provider)
