...
See [Source Tiers & Reliability](upstream/nature-academic-search/references/source-tiers.md) for the complete reliability classification and fallback routing rules. The T1→T2→T3 fallback chain is the standard execution order across all workflows.
...
- **Discovery** — `scripts/academic_search.py` queries OpenAlex. Anonymous demo requests may work, but normal product use should set the free `OPENALEX_API_KEY`. OpenAlex indexes CrossRef, PubMed and arXiv-deposited works, so one endpoint covers journals and preprints for keyword/author search, with relevance re-ranking and author disambiguation (`--affiliation` / `--orcid` / `--list-authors`). Returns ranked JSON (title, DOI, authors, year, citations, abstract).
