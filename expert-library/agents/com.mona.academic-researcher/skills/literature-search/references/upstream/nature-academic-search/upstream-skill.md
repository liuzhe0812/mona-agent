...
- A **static layer** under `upstream/nature-academic-search/static/` that holds versioned, reusable content fragments (the MCP tool inventory and shared modules, and source routing plus operational rules).
- A **dynamic layer** (this file plus `upstream/nature-academic-search/manifest.yaml`) that detects which workflow the user needs and loads that workflow, reaching for shared modules and scripts only when a step needs them.
...
Read [manifest.yaml](upstream/nature-academic-search/manifest.yaml). It declares the `workflow` axis, the allowed values, and the file paths each value maps to.

Also read every file listed under `always_load`:

- `upstream/nature-academic-search/static/core/tools.md` — the MCP tool inventory (core search, extended search, PubMed utilities) and the shared-module map.
- `upstream/nature-academic-search/static/core/routing-and-ops.md` — the T1→T2→T3 source routing quick guide, environment setup, error handling, and limitations.
...
Read the file mapped for each detected workflow (under `upstream/nature-academic-search/references/workflows/`). Do **not** read every workflow. Each workflow file links to the shared modules it needs.
...
The files under `upstream/nature-academic-search/references/` (and `scripts/`) are deep references, not defaults. Open them on demand per the `references.on_demand` table in the manifest — for example `upstream/nature-academic-search/references/source-tiers.md` for the full reliability classification, `upstream/nature-academic-search/references/dedup-engine.md` / `upstream/nature-academic-search/references/citation-parser.md` / `upstream/nature-academic-search/references/search-strategy.md` / `upstream/nature-academic-search/references/ris-bibtex-format.md` for the shared modules, and `scripts/academic_search.py` (no-MCP fallback discovery search) / `scripts/format-converter.py` / `scripts/preflight.py` for the tooling.
