...
4. There is no visible QR-only login, SMS/OTP field, push-approval prompt, password reset prompt, consent-to-share-new-data prompt, or account/security warning. Follow the boundaries in `upstream/nature-downloader/upstream-skill.md`; Mona does not bypass CAPTCHA or access control.
