...
- A **static layer** under `upstream/nature-data/static/` that holds versioned, reusable content fragments (the default stance and source hierarchy, the Chinese-user operating mode, and the workflow with output format).
- A **dynamic layer** (this file plus `upstream/nature-data/manifest.yaml`) that loads the core every time and reaches for the deeper policy/repository/FAIR references only when a step needs them.
...
Read [manifest.yaml](upstream/nature-data/manifest.yaml). Then read every file listed under `always_load`:

- `upstream/nature-data/static/core/stance.md` — what the data-availability package is, the default stance, and the source hierarchy.
- `upstream/nature-data/static/core/chinese-mode.md` — how to operate when the user writes in Chinese (accept Chinese, draft English, convert terms precisely).
- `upstream/nature-data/static/core/workflow.md` — the eight-step workflow and the output format.
...
The files under `upstream/nature-data/references/` are deep references, not defaults. Open them on demand per the `references.on_demand` table in the manifest — for example `upstream/nature-data/references/policy-principles.md` for the governing rules and edge cases, `upstream/nature-data/references/repository-and-identifiers.md` for repository/accession/DOI choices, `upstream/nature-data/references/statement-patterns.md` for ready-to-adapt statements, `upstream/nature-data/references/fair-metadata-checklist.md` for the FAIR audit, `upstream/nature-data/references/chinese-author-alignment.md` for Chinese wording, and `upstream/nature-data/references/source-basis.md` to justify a rule with its official source.

When the target is the flagship journal Nature, also open
`upstream/nature-data/references/nature-article-requirements.md` for statement placement,
...
`upstream/nature-shared/journal-formats/nature-machine-intelligence.md`. Enforce a
