...
Authoritative facts about Nature Communications formatting requirements. Used by both `nature-polishing` and `nature-writing` when `journal=nat-comms`. This file holds the **facts**; each skill's `upstream/nature-shared/static/fragments/journal/nat-comms.md` adds the **action layer**.
