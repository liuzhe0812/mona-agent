...
- A **static layer** under `upstream/nature-figure/static/` that holds versioned, reusable content fragments (the figure contract and default stance, plus a per-backend quick-start for Python and R).
- A **dynamic layer** (this file plus `upstream/nature-figure/manifest.yaml`) that detects the plotting backend and loads only the fragment needed for the current job. The large design, API, pattern, and QA material lives in on-demand references.
...
uses AI, read
[references/ai-graphical-abstract-workflow.md](upstream/nature-figure/references/ai-graphical-abstract-workflow.md)
...
1. Read [manifest.yaml](upstream/nature-figure/manifest.yaml) and the `always_load` files.
2. Read [references/ai-graphical-abstract-workflow.md](upstream/nature-figure/references/ai-graphical-abstract-workflow.md).
3. Read [references/openrouter-image-generation.md](upstream/nature-figure/references/openrouter-image-generation.md).
...
Read [manifest.yaml](upstream/nature-figure/manifest.yaml). It declares the `backend` axis, the allowed values, and the file paths each value maps to.

Also read every file listed under `always_load` (`upstream/nature-figure/static/core/contract.md` and `upstream/nature-figure/static/core/stance.md`). These hold the figure contract, the backend gate, the missing-runtime rule, the privacy rule, and the default operating stance that apply to every figure job.
...
2. Multi-panel evidence architecture — when planning, restructuring, or auditing a labelled multi-panel figure, load `upstream/nature-figure/references/multipanel-evidence-architecture.md`. Make the figure answer one Results-level scientific question; assign panels different inferential roles, not merely different metrics. When figure order must follow the manuscript argument, also load `upstream/nature-shared/core/nature-results-discussion.md`.
...
5. Template adaptation — when reusing built-in original examples, licensed external material, or user-provided plotting code, load `upstream/nature-figure/references/asset-adaptation.md` before mapping data or changing the script.
6. Rendered QA and delivery preflight — load `upstream/nature-figure/references/qa-contract.md`, run the render-time panel-alignment gate for every multi-panel figure, `scripts/validate_figure.py` on the plotting source, `scripts/audit_pdf_text.py` on the exported PDF, and `scripts/audit_figure_collisions.py` on the same final PDF. Then inspect every panel and the complete figure at final physical size. Automated checks do not replace the panel-by-panel uncertainty, salience, spacing, and ambiguity audit.
...
`upstream/nature-figure/references/nature-article-requirements.md`. It separates initial-review files
...
`upstream/nature-shared/journal-formats/nature-machine-intelligence.md`. Apply its
...
The files under `upstream/nature-figure/references/` are deep references, not defaults. Open them on demand per the `references.on_demand` table in the manifest — for example `upstream/nature-figure/references/figure-contract.md` to build the contract, `upstream/nature-figure/references/multipanel-evidence-architecture.md` to turn one Results-level question into complementary panel roles and a claim-escalating figure sequence, `upstream/nature-figure/references/asset-adaptation.md` to reuse a plotting template safely, `upstream/nature-figure/references/template-catalog.md` for validated Python CSV templates, `upstream/nature-figure/references/api.md` for the Python palette and numerical/layout safety helpers, `upstream/nature-figure/references/r-workflow.md` for R, `upstream/nature-figure/references/design-theory.md` for color/typography/export rationale, `upstream/nature-figure/references/common-patterns.md` and `upstream/nature-figure/references/chart-types.md` for layout/chart recipes, `upstream/nature-figure/references/nature-2026-observations.md` for real Nature page archetypes, `upstream/nature-figure/references/qa-contract.md` before final delivery, `upstream/nature-figure/references/nature-article-requirements.md` for exact flagship Nature stage and upload rules, `upstream/nature-shared/journal-formats/nature-machine-intelligence.md` for exact NMI figure rules, `upstream/nature-figure/references/ai-graphical-abstract-workflow.md` for AI-assisted graphical-abstract planning, policy gating, human verification, and provenance, and `upstream/nature-figure/references/tutorials.md` / `upstream/nature-figure/references/demos.md` for worked examples.
