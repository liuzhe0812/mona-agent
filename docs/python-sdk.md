...
Use mona as a library — no CLI, no gateway, just Python.
...
`mona.from_config()` reuses your normal `~/.mona/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying mona internals:

```python
from mona.agent import AgentHook, AgentHookContext
...
### `mona.from_config(config_path=None, *, workspace=None)`

Create a `mona` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.mona/config.json`. |
...
```python
from mona.agent import AgentHook
...
async def main() -> None:
    bot = mona.from_config(workspace="/my/project")
