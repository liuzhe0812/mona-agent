...
When someone sends a DM to the bot and isn't on the allowlist — whether it's a new user or an existing user on a new channel — mona automatically replies with a **pairing code** (like `ABCD-EFGH`) that expires in 10 minutes. To grant them access:
...
The gateway wakes up every 30 minutes and checks `HEARTBEAT.md` in your workspace (`~/.mona/workspace/HEARTBEAT.md`). If the file has tasks, the agent executes them and delivers results to your most recently active chat channel.

**Setup:** edit `~/.mona/workspace/HEARTBEAT.md` (created automatically by `mona onboard`):
...
> **Note:** The gateway must be running (`mona gateway`) and you must have chatted with the bot at least once so it knows which channel to deliver to.
