...
Build a custom mona channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of mona (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

mona discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `mona gateway` starts, it scans:

1. Built-in channels in `mona/channels/`
2. External packages registered under the `mona.channels` entry point group
...
```text
mona-channel-webhook/
├── mona_channel_webhook/
...
```python
# mona_channel_webhook/__init__.py
from mona_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# mona_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "mona-channel-webhook"
version = "0.1.0"
dependencies = ["mona-ai", "aiohttp"]

[project.entry-points."mona.channels"]
webhook = "mona_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
mona plugins list      # verify "Webhook" shows as "plugin"
mona onboard           # auto-adds default config for detected plugins
```

Edit `~/.mona/config.json`:
...
```bash
mona channels login <channel_name>
mona channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `mona onboard`. Override to declare your fields. |
...
Besides normal assistant text, mona can emit low-emphasis trace blocks. These are intended for UI affordances like status rows, collapsible "used tools" groups, or reasoning/thinking blocks. Platforms that do not have a good place for them can ignore them safely.
...
Built-in channels use Pydantic config models (subclassing `Base` from `mona.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `mona.config.schema.Base`:
...
Override `default_config()` so `mona onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `mona-channel-{name}` | `mona-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `mona_channel_{name}` | `mona_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/mona-channel-webhook
cd mona-channel-webhook
pip install -e .
mona plugins list    # should show "Webhook" as "plugin"
mona gateway         # test end-to-end
```
