...
mona can generate and edit images through the `generate_image` tool. In the WebUI, users can enable **Image Generation** from the composer, choose an aspect ratio, and keep iterating on generated images inside the same chat.

The feature is disabled by default. Enable it in `~/.mona/config.json`, configure a supported image provider, then restart the gateway.
...
> [!TIP]
> Prefer environment variables for API keys. mona resolves `${VAR_NAME}` values from the environment at startup.
...
| `tools.imageGeneration.saveDir` | string | `"generated"` | Relative directory under mona's media directory for generated artifacts |
...
AIHubMix `gpt-image-2-free` is supported through AIHubMix's unified predictions API. Internally mona calls:
...
mona supports two Gemini image generation model families via Google's Generative Language API:
...
Generated images are stored under the active mona instance's media directory:

```text
~/.mona/media/generated/YYYY-MM-DD/img_<id>.<ext>
~/.mona/media/generated/YYYY-MM-DD/img_<id>.json
```
...
```text
A minimal app icon for mona: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text
```
...
| AIHubMix says `Incorrect model ID` | Use `model: "gpt-image-2-free"`; mona expands it to the required `openai/gpt-image-2-free` model path internally |
| Generation times out | Try a smaller/default image size, set AIHubMix `extraBody.quality` to `"low"`, or retry later |
| Reference image rejected | Reference image paths must be inside the workspace or mona media directory and must be valid image files |
