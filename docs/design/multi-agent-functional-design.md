...
5. **工作流不占用全局功能导航。** 用户在房间右侧栏查看工作流摘要，在主区域完成编辑。
6. **第一版不做自由连线画布。** 使用“自然语言生成 + 纵向步骤卡片 + 并行步骤组”的编辑方式；底层保留有向无环依赖模型，未来可增加画布作为同一数据的另一种视图。
...
| Agent Package | 可安装、升级、签名校验的不可变 Agent 产品包 | 商店与伙伴详情显示 |
...
| F08 | P0 | 自然语言编排 | 用户描述协作目标后，Mona 生成结构化工作流草稿；草稿必须经用户检查和保存才能成为当前工作流。 |
...
| F18 | P2 | Agent Package | 从本地包或官网安装签名 Agent Package，显示权限变化，保持产品文件和用户数据分离。 |
...
- Mona 可以显式创建专业 Agent Job。
- 专业 Agent 第一版不能委派其他 Agent。
...
用户或 Mona 可以在房间内创建一次性 Job，例如：

> @A股分析师，补充分析这家公司最近一季现金流。
...
3. Mona 校验官方签名、文件哈希、目录路径、包大小和 Schema。
...
- [ ] Agent Package 未通过签名和路径安全校验时不能安装。
...
- 第三方 Agent 开发者市场：先完成官方签名包和权限模型，再开放第三方发布。
