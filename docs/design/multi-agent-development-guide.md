...
> 可复用或定时流程：用户在 Mona 私聊创建协作房间 → Mona 生成工作流草稿 → 用户保存并运行 → A 股分析师完成自己的步骤 → Mona 使用分析结果继续执行 → 用户审批 → 所有状态、消息和产物回到原房间 → 重启后仍可查看和恢复。即时协作则由结构化 `@` 与明确自然语言意图直接启动一次性 WorkflowRun。
...
- 第一版只允许 Mona 通过 `delegate_agent` 创建单层命名 Agent Job，或作为一次性协作的编排者；专业 Agent 不注册委派工具，也不能通过回复文本中的 `@` 触发其他 Agent。
...
- 没有结构化 `target_agent_ids` 时才走 Mona 默认消息流程。
- 有结构化目标时只唤醒目标 Agent；默认多个目标并行执行，只 `@` 专业 Agent 不额外唤醒 Mona，只 `@Mona` 则走正常 Mona 流程。
- 明确出现“先……再”“先……然后”或“先……接着”时创建一次性串行运行；明确出现“复核”“审查”“评审”或“检查”时先产出、后复核，后续步骤依赖第一个步骤。
- 只有结构化 targets 包含 Mona，且存在“总结”“汇总”或“综合”意图时，伙伴并行产出后才运行 Mona 唯一汇总；partner-only 请求保持 parallel。
- `@Mona` 与专业 Agent 同时出现时，默认 parallel 模式下分别回答；串行、复核或汇总模式则由一次性 WorkflowRun 统一执行，避免重复派发和普通 Mona 主回复。
- 即时串行、复核和汇总均持久化完整 WorkflowRun 快照，但不创建或激活可复用 workflow draft；可复用或定时流程仍调用 `propose_workflow`，由用户检查、保存和激活。
- 多个 parallel 目标使用同一份消息开始时的房间上下文快照；每个专业 Agent 的任务必须限定为只回答自己的部分，不代表其他被 `@` 成员。
- Agent 回复正文中的普通 `@` 只是文本，不触发执行；只有受控的 `delegate_agent` 动作可以创建 Job。
...
一次性协作的启动约束：路由器根据结构化 targets 和明确意图构建临时 WorkflowDefinition，调用现有 WorkflowRunner 启动手动 WorkflowRun；用户消息只持久化一次，启动校验或运行冲突必须作为已处理错误返回，不能回退到旧的直接派发路径。
...
- 已完成步骤和产物保留。
- 迟到回调先检查当前状态。
- 运行中的单个步骤可以单独取消，不影响已经成功的兄弟步骤或已完成的上游结果。
...
- 失败步骤支持单步重试；重试只创建新的 attempt，成功的兄弟步骤和已完成的上游结果保留，不重复执行无关步骤。
...
- 目标必须是当前房间成员且处于启用状态。
- 禁止目标为当前 Agent。
- 第一版只允许 Mona 做单层委派；专业 Agent 不能委派，且不允许 parent_job 再创建 child_job。
- 第一版不承诺递归委派、Agent 自由辩论或投票；专业 Agent 互相委派不在本次范围。
- 直接 `@` 已由路由器派发的 Agent 不得被 Mona 重复委派。
...
- [x] 房间当前工作流和草稿。
- [x] 纵向步骤卡片编辑器。
- [x] 可复用或定时流程的自然语言生成结构化草稿工具；即时协作由路由器直接生成一次性 WorkflowRun。
