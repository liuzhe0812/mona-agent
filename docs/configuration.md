...
Config file: `~/.mona/config.json`

> [!NOTE]
> If your config file is older than the current schema, you can refresh it without overwriting your existing values:
> run `mona onboard`, then answer `N` when asked whether to overwrite the config.
> mona will merge in missing default fields and keep your current settings.
...
Any string value in `config.json` can use `${VAR_NAME}`. Resolution runs once at startup, in memory only — resolved values are never written back to disk, so editing config through `mona onboard` or the WebUI preserves the placeholder.

If a referenced variable is unset, mona fails fast at startup with `ValueError: Environment variable 'NAME' referenced in config is not set`.
...
Pick whatever fits your deployment — mona only reads `os.environ` at startup, so any mechanism that populates the process environment works.

**systemd** — use `EnvironmentFile=` in the service unit to load variables from a file that only the deploying user can read:

```ini
# /etc/systemd/system/mona.service (excerpt)
[Service]
EnvironmentFile=/home/youruser/mona_secrets.env
User=mona
ExecStart=...
```

```bash
# /home/youruser/mona_secrets.env (mode 600, owned by youruser)
...
```bash
docker run --rm --env-file=./mona.env \
  -v ~/.mona:/home/mona/.mona \
  mona agent -m "Hello"
```
...
op run --env-file=.env.tpl -- mona agent

# pass (passwordstore.org)
ANTHROPIC_API_KEY="$(pass show api/anthropic)" mona agent

# Bitwarden
ANTHROPIC_API_KEY="$(bw get password api/anthropic)" mona agent
```
...
> - **MiniMax Coding Plan**: Exclusive discount links for the mona community: [Overseas](https://platform.minimax.io/subscribe/coding-plan?code=9txpdXw04g&source=link) · [Mainland China](https://platform.minimaxi.com/subscribe/token-plan?code=GILTJpMTqZ&source=link)
> - **MiniMax (Mainland China)**: If your API key is from MiniMax's mainland China platform (minimaxi.com), set `"apiBase": "https://api.minimaxi.com/v1"` in your minimax provider config.
> - **MiniMax thinking mode**: Use `providers.minimaxAnthropic` when you want `reasoningEffort` / thinking mode. MiniMax exposes that capability through its Anthropic-compatible endpoint, so mona keeps it as a separate provider instead of guessing MiniMax-specific thinking parameters on the generic OpenAI-compatible `minimax` endpoint. It uses the same `MINIMAX_API_KEY`. Default Anthropic-compatible base URL: `https://api.minimax.io/anthropic`; for mainland China use `https://api.minimaxi.com/anthropic`.
...
| `volcengine` | LLM (VolcEngine, pay-per-use) | [Coding Plan](https://www.volcengine.com/activity/codingplan?utm_campaign=mona&utm_content=mona&utm_medium=devrel&utm_source=OWO&utm_term=mona) · [volcengine.com](https://www.volcengine.com) |
| `byteplus` | LLM (VolcEngine international, pay-per-use) | [Coding Plan](https://www.byteplus.com/en/activity/codingplan?utm_campaign=mona&utm_content=mona&utm_medium=devrel&utm_source=OWO&utm_term=mona) · [byteplus.com](https://www.byteplus.com) |
...
| `openai_codex` | LLM (Codex, OAuth) | `mona provider login openai-codex` |
| `github_copilot` | LLM (GitHub Copilot, OAuth) | `mona provider login github-copilot` |
...
You can also set `providers.bedrock.apiKey` to a Bedrock API key; mona exports it as `AWS_BEARER_TOKEN_BEDROCK` for the AWS SDK.
...
Claude Opus 4.7 does not accept `temperature`, `top_p`, or `top_k`; mona omits `temperature` automatically for this model. If `reasoningEffort` is set to `low`, `medium`, `high`, `max`, or `adaptive`, mona sends Bedrock's adaptive thinking parameter.
...
Use Bedrock model IDs or inference profile IDs with a `bedrock/` prefix in mona config. mona removes the prefix before calling AWS.
...
Model-specific fields can be supplied with `extraBody`; mona merges it into Converse `additionalModelRequestFields`:
...
Current scope: mona passes `messages`, `system`, `inferenceConfig`, `toolConfig`, and `additionalModelRequestFields`. Bedrock Prompt Management, Guardrails, `serviceTier`, and other top-level Converse options are not first-class config fields yet.
...
```bash
mona agent -m "Reply with one short sentence."
```
...
No `providers.openaiCodex` block is needed in `config.json`; `mona provider login` stores the OAuth session outside config.

**1. Login:**
```bash
mona provider login openai-codex
```

**2. Set model** (merge into `~/.mona/config.json`):
```json
{
  "agents": {
    "defaults": {
...
mona agent -c ~/.mona-telegram/config.json -m "Hello!"

# One-off workspace override on top of that config
mona agent -c ~/.mona-telegram/config.json -w /tmp/mona-telegram-test -m "Hello!"
```
...
No `providers.githubCopilot` block is needed in `config.json`; `mona provider login` stores the OAuth session outside config.

**1. Login:**
```bash
mona provider login github-copilot
```
...
LongCat is available through mona's built-in OpenAI-compatible provider flow.
...
Ant Ling is available through mona's built-in OpenAI-compatible provider flow.
...
Some OpenAI-compatible gateways expose request-body extensions such as vLLM guided decoding or local sampling controls. Put those under `extraBody`; mona merges them into the chat-completions request body after its provider defaults:
...
**2. Add to config** (partial — merge into `~/.mona/config.json`):
```json
{
  "providers": {
    "ollama": {
...
[Atomic Chat](https://atomic.chat/) is a local-first desktop app that exposes an **OpenAI-compatible** HTTP API (default `http://localhost:1337/v1`). Use it when you want to run mona against a model on your own machine instead of a hosted API provider.
...
**3. Add to config** (partial — merge into `~/.mona/config.json`):
...
mona uses a **Provider Registry** (`mona/providers/registry.py`) as the single source of truth.
Adding a new provider only takes **2 steps** — no if-elif chains to touch.

**Step 1.** Add a `ProviderSpec` entry to `PROVIDERS` in `mona/providers/registry.py`:
...
ProviderSpec(
    ...
    display_name="My Provider",          # shown in `mona status`
    default_api_base="https://api.myprovider.com/v1",  # OpenAI-compatible endpoint
)
```

**Step 2.** Add a field to `ProvidersConfig` in `mona/config/schema.py`:
...
That's it! Environment variables, model routing, config matching, and `mona status` display will all work automatically.
...
Existing configs do not need to change. If you do not set `modelPresets` or `agents.defaults.modelPreset`, mona keeps using `agents.defaults.*` exactly as before.
...
If fallback candidates use smaller `contextWindowTokens` values, mona builds context using the smallest window in the active chain so every candidate can receive the same prompt.
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.mona/config.json`:
...
When a channel `send()` raises, mona retries at the channel-manager layer. By default, `channels.sendMaxRetries` is `3`, and that count includes the initial send.
...
>
> If a channel is completely unreachable, mona cannot notify the user through that same channel. Watch logs for `Failed to send to {channel} after N attempts` to spot persistent delivery failures.

## Web Tools

mona incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.mona/config.json` under `tools.web`.
...
mona supports multiple web search providers. Configure in `~/.mona/config.json` under `tools.web.search`.
...
mona by default uses [Jina Reader](https://jina.ai/reader/), a third-party API, to convert arbitrary pages into Markdown format for easy digestion by the LLM, with a local fallback based on [readability-lxml](https://github.com/buriy/python-readability) if the former fails.
...
mona supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped mona tool name (for example `mcp_filesystem_write_file`).
...
**Docker security**: The official Docker image runs as a non-root user (`mona`, UID 1000) with bubblewrap pre-installed. When using `docker-compose.yml`, the container drops all Linux capabilities except `SYS_ADMIN` (required for bwrap's namespace isolation).
...
```bash
mona agent -m "/pairing list"
mona agent -m "/pairing approve ABCD-EFGH"
```


## Subagent Concurrency

By default, mona only allows one spawned subagent at a time. When the limit is
...
When a user is idle for longer than a configured threshold, mona **proactively** compresses the older part of the session context into a summary while keeping a recent legal suffix of live messages. This reduces token cost and first-token latency when the user returns — instead of re-processing a long stale context with an expired KV cache, the model receives a compact summary, the most recent live context, and fresh input.
...
By default, mona uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
By default, each channel × chat ID combination gets its own session. If you use mona across multiple channels (e.g. Telegram + Discord + CLI) and want them to share the same conversation, enable `unifiedSession`:
...
mona ships with built-in skills, and your workspace can also define custom skills under `skills/`. If you want to hide specific skills from the agent, set `agents.defaults.disabledSkills` to a list of skill directory names:
