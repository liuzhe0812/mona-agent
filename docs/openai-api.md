...
mona can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "mona-ai[api]"
mona serve
```
...
If `channel` points to a channel that is not enabled in your config, mona will queue the outbound event but no platform delivery will occur.
