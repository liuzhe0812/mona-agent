...
- GET  /payment/return               支付宝同步返回确认页
- GET  /payment/alipay/return        支付宝同步返回兼容入口
"""
...
from app.recharge_promotion import grant_recharge_pro_bonus
...
def _to_UTC(dt: datetime | None) -> datetime | None:  # noqa: N802
    if dt is None:
        return None
...
def create_subscription(
    ...
    if body.payment_method == "alipay_periodic":
        ...
        if not result.success:
            ...
            db.commit()
            logger.error("Alipay sign-and-pay failed for %s", payment.trade_order_id)
            raise AuthError(
                "alipay_sign_failed",
        ...
        try:
            ...
            raise AuthError("alipay_pay_failed", "Unable to create payment", 502) from e
...
def _validate_trade_notification(
    db: Session,
    data: dict[str, Any],
    *,
    ...
    if data.get("app_id") != settings.alipay_app_id:
        raise ValueError("Alipay app_id mismatch")
    if not settings.alipay_seller_id:
        raise ValueError("Alipay seller_id is not configured")
    if data.get("seller_id") != settings.alipay_seller_id:
        raise ValueError("Alipay seller_id mismatch")
    try:
        notified_amount = Decimal(data.get("total_amount", ""))
    except InvalidOperation as exc:
        raise ValueError("Invalid Alipay total_amount") from exc
    if notified_amount != expected_amount:
        raise ValueError("Alipay total_amount mismatch")
    if not trade_no:
        raise ValueError("Missing Alipay trade_no")
    duplicate = db.query(Payment).filter(Payment.alipay_trade_no == trade_no)
    if payment_id is not None:
        duplicate = duplicate.filter(Payment.id != payment_id)
    if duplicate.first() is not None:
        raise ValueError("Alipay trade_no is already attached to another order")


def _fulfill_payment(db: Session, payment: Payment, trade_no: str) -> None:
    if payment.fulfillment_status == "succeeded":
        if payment.alipay_trade_no != trade_no:
            raise ValueError("Fulfilled order has a different Alipay trade_no")
        return
    payment.status = PaymentStatus.PAID
    payment.paid_at = _now()
    payment.alipay_trade_no = trade_no

    if payment.product_type == "credit_topup":
        if not payment.credit_units or payment.credit_units <= 0:
            raise ValueError("Credit order has invalid credit_units")
        grant_topup(
            db,
            user_id=payment.user_id,
            units=payment.credit_units,
            reference_id=f"payment:{payment.trade_order_id}",
            metadata={"product_code": payment.product_code, "payment_id": payment.id},
        )
        grant_recharge_pro_bonus(db, payment, now=payment.paid_at)
    else:
        ...
        if plan:
            ...
            if payment.agreement_no:
                agreement = (
                    db.query(PaymentAgreement)
                    .filter(PaymentAgreement.agreement_no == payment.agreement_no)
                    .first()
                )
            _activate_subscription(
                ...
                payment.user_id,
                plan,
                auto_renew=payment.payment_type == "periodic_sign",
...
async def alipay_notify(request: Request, db: Session = Depends(get_db)):
    ...
    if data.get("sign_type") != "RSA2":
        logger.warning("Alipay notify rejected unsupported signature type")
        return "fail"
    if not alipay_service.verify_callback(data.copy()):
        logger.warning(
            "Alipay notify signature verification failed for order %s",
            data.get("out_trade_no", ""),
        )
        return "fail"
    if data.get("app_id") != settings.alipay_app_id:
        logger.warning("Alipay notify app identity mismatch")
        return "fail"
    ...
    logger.info(
        "Alipay notify: notify_type=%s trade_status=%s out_trade_no=%s",
        notify_type,
        trade_status,
        out_trade_no,
    )
    ...
    try:
        if notify_type == "cycle_sign":
            ...
            logger.info(
                "Unhandled alipay notify: type=%s status=%s order=%s",
                notify_type,
                trade_status,
                out_trade_no,
            )
...
async def _handle_cycle_sign_notify(db: Session, data: dict[str, Any]) -> None:
    ...
    payment = db.query(Payment).filter(Payment.external_sign_no == external_sign_no).first()
    ...
    if status == "VERIFIED":
        ...
        if not agreement_no:
            ...
            return
        existing = (
            db.query(PaymentAgreement).filter(PaymentAgreement.agreement_no == agreement_no).first()
        )
        if existing:
            # 已处理，幂等
            return
        ...
        db.commit()
        logger.info(
            "Agreement created: %s for user %s", agreement_no, payment.user_id if payment else "?"
        )
...
async def _handle_cycle_charge_notify(db: Session, data: dict[str, Any], trade_no: str) -> None:
    ...
    renewal = (
        db.query(SubscriptionRenewal)
        .filter(SubscriptionRenewal.out_trade_no == out_trade_no)
        .with_for_update()
        .first()
    )
    if renewal:
    ...
    # 可能是首期扣款（签约并支付）
    payment = (
        db.query(Payment).filter(Payment.trade_order_id == out_trade_no).with_for_update().first()
    )
    if payment:
        _validate_trade_notification(
            db,
        ...
        _fulfill_payment(db, payment, trade_no)
        db.commit()
        return

    logger.warning("cycle_charge notify for unknown out_trade_no: %s", out_trade_no)
    raise ValueError("Unknown cycle charge order")
...
_PAYMENT_RETURN_HTML = """<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Mona 支付结果确认</title>
  <style>
...
<body>
  <main>
    <h1>支付结果正在确认，请返回 Mona 查看</h1>
    <p>支付完成后，Mona 会自动更新充值状态。</p>
...
@router.get("/return", response_class=HTMLResponse, name="alipay_return")
@router.get("/alipay/return", response_class=HTMLResponse, name="alipay_return_legacy")
async def alipay_return():
    """支付宝同步返回确认页，不根据 query 参数更新订单。"""
    ...
    return HTMLResponse(
        content=_PAYMENT_RETURN_HTML.replace("__CSP_NONCE__", nonce),
        headers=headers,
    )
...
def sync_payment_status(db: Session, payment: Payment, *, strict: bool = False) -> None:
    """主动查询支付宝订单状态并同步"""
    if not alipay_service.enabled:
        if strict:
            raise RuntimeError("Payment status synchronization is unavailable")
        return
    if payment.fulfillment_status == "succeeded":
        return
    try:
        ...
        if strict:
            raise RuntimeError("Payment provider status query failed") from None
        return

    if strict and result.get("code") != "10000":
        if result.get("code") == "40004" and result.get("sub_code") == "ACQ.TRADE_NOT_EXIST":
            return
        raise RuntimeError("Payment provider rejected status query")
    ...
    if trade_status in ("TRADE_SUCCESS", "TRADE_FINISHED") and result.get("code") == "10000":
        ...
        _fulfill_payment(db, payment, trade_no)
        db.commit()
    elif trade_status == "TRADE_CLOSED" and result.get("code") == "10000":
        payment = (
            db.query(Payment)
            .filter(Payment.id == payment.id)
            .populate_existing()
        ...
        if payment.status == PaymentStatus.PENDING:
            payment.status = PaymentStatus.FAILED
            db.commit()
    elif strict and trade_status != "WAIT_BUYER_PAY":
        raise RuntimeError("Payment provider returned an unknown trade status")
