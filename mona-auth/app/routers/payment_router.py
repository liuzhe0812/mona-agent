...
from app.plans import subscription_period_days
...
def _activate_subscription(
    db: Session, user_id: int, duration_months: int, plan_code: str | None = None
):
...
async def payment_notify(request: Request, db: Session = Depends(get_db)):
    ...
    if status_val == "OD" and payment.status != PaymentStatus.PAID:
        ...
        _activate_subscription(db, payment.user_id, payment.duration_months, payment.plan_code)
