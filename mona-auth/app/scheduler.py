...
def _process_subscription_deduction(db: Session, sub: Subscription, now: datetime) -> None:
    ...
    plan = db.query(PricingPlan).filter(PricingPlan.id == sub.plan_code).first() if sub.plan_code else None
    if not plan:
    ...
    if existing and existing.status in (RenewalStatus.PENDING, RenewalStatus.SUCCESS, RenewalStatus.RETRYING):
        return  # 本周期已发起扣款
    ...
    if result.success:
        ...
        if sub.current_period_end:
            ...
            sub.status = SubscriptionStatus.ACTIVE
        db.commit()
        logger.info("Renewal success for subscription %s, extended to %s", sub.id, sub.current_period_end)
    else:
...
def start_scheduler() -> None:
    ...
    _scheduler.start()
    logger.info("Scheduler started: auto_deduct, retry_deductions, pre_deduct_notify, expire_subscriptions")
