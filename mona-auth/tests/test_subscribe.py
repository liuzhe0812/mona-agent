...
class TestAlipayReturn:
    def test_return_paths_render_confirmation_without_fulfilling(self, client, db):
        user = User(email="return@test.com", account="returnuser", password_hash="x")
        db.add(user)
        db.commit()
        payment = Payment(
            user_id=user.id,
            trade_order_id="return-order",
            amount=1.0,
        ...
        db.add(payment)
        db.commit()
        marker = "return-marker-<img src=x onerror=alert(1)>"

        for path in ("/payment/return", "/payment/alipay/return"):
            response = client.get(
                path,
                params={"trade_status": "TRADE_SUCCESS", "out_trade_no": marker},
            ...
            assert "支付结果正在确认，请返回 Mona 查看" in response.text
            assert "window.close()" in response.text
            assert "手动关闭此页面" in response.text
